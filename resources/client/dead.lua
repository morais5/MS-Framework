	...
	while true do
		...
		if NetworkIsPlayerActive(player) then
            ...
            elseif IsEntityDead(playerPed) and InLaststand and not isDead then
                ...
                TriggerServerEvent("cash-log:server:CreateLog", "death", GetPlayerName(player) .. " ("..GetPlayerServerId(player)..") is dead", "red", "**".. killerName .. "** has murdered ".. GetPlayerName(player) .." with **".. weaponLabel .. "** (" .. weaponName .. ")")
		...
		if isDead or InLaststand then
            ...
            if isDead then
                if not isInHospitalBed then 
                    if deathTime > 0 then
                        DrawTxt(0.93, 1.44, 1.0,1.0,0.6, "RESPAWN EM: ~b~" .. math.ceil(deathTime) .. "~r~ SEGUNDOS", 255, 255, 255, 255)
                    else
                        DrawTxt(0.865, 1.44, 1.0, 1.0, 0.6, "~w~ Espera ~b~[E] ("..hold..")~w~ CARREGA PARA RENASCER ~r~($"..Config.BillCost..")~w~", 255, 255, 255, 255)
                    end
                end
            ...
            elseif InLaststand then
                ...
                if LaststandTime > Laststand.MinimumRevive then
                    DrawTxt(0.94, 1.44, 1.0, 1.0, 0.6, "Sangrará em: ~b~" .. math.ceil(LaststandTime) .. "~r~ segundos", 255, 255, 255, 255)
                else
                    DrawTxt(0.845, 1.44, 1.0, 1.0, 0.6, "Sangrará em: ~b~" .. math.ceil(LaststandTime) .. "~r~ segundos, Ainda Pode ser reanimado", 255, 255, 255, 255)
                end
