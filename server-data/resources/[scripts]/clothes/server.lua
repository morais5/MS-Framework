...
AddEventHandler('clothes:saveSkin', function(model, skin)
    ...
    if model ~= nil and skin ~= nil then 
        MSCore.Functions.ExecuteSql(false, "DELETE FROM `playerskins` WHERE `citizenid` = '"..Player.PlayerData.citizenid.."'", function(result)
...
AddEventHandler('clothes:saveOutfit', function(model, skin, outfitname)
    ...
    local Player = MSCore.Functions.GetPlayer(src)
    if model ~= nil and skin ~= nil then
        MSCore.Functions.ExecuteSql(false, "INSERT INTO `player_outfits` (`citizenid`, `outfitname`, `model`, `skin`) VALUES ('"..Player.PlayerData.citizenid.."', '"..outfitname.."', '"..model.."', '"..skin.."')")
    end
end)
...
AddEventHandler('clothes:removeOutfit', function(outfitname)
    ...
    MSCore.Functions.ExecuteSql(false, "DELETE FROM `player_outfits` WHERE `citizenid` = '"..Player.PlayerData.citizenid.."' AND `outfitname` = '"..outfitname.."'")
end)
...
MSCore.Commands.Add("h", "Put your helmet/cap/hat on or off..", {}, false, function(source, args)
...
MSCore.Commands.Add("g", "Put your glasses on or off..", {}, false, function(source, args)
	TriggerClientEvent("clothes:client::adjustfacewear", source, 2) -- Glasses
end)

MSCore.Commands.Add("m", "Put your mask on or off..", {}, false, function(source, args)
	--TriggerClientEvent("clothes:client::adjustfacewear", source, 3)
