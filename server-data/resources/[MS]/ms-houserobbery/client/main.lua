...
AddEventHandler('MSCore:Client:OnPlayerLoaded', function()
    MSCore.Functions.TriggerCallback('ms-houserobbery:server:GetHouseConfig', function(HouseConfig)
...
Citizen.CreateThread(function()
    Citizen.Wait(500)
    requiredItems = {
        [1] = {name = MSCore.Shared.Items["lockpick"]["name"], image = MSCore.Shared.Items["lockpick"]["image"]},
        [2] = {name = MSCore.Shared.Items["screwdriverset"]["name"], image = MSCore.Shared.Items["screwdriverset"]["image"]},
    }
    while true do
        inRange = false
...
function enterRobberyHouse(house)
    ...
    if Config.Houses[house]["tier"] == 1 then
        data = exports['ms-interior']:CreateTier1HouseFurnished(coords)
    end
    Citizen.Wait(100)
    houseObj = data[1]
    POIOffsets = data[2]
...
RegisterNetEvent('ms-houserobbery:client:ResetHouseState')
AddEventHandler('ms-houserobbery:client:ResetHouseState', function(house)
...
RegisterNetEvent('ms-houserobbery:client:enterHouse')
AddEventHandler('ms-houserobbery:client:enterHouse', function(house)
            ...
            if closestHouse ~= nil then
                if CurrentCops >= Config.MinimumHouseRobberyPolice then
                    if not Config.Houses[closestHouse]["opened"] then
                        ...
                        MSCore.Functions.Notify('The door is already open..', 'error', 3500)
                    end
                else
                    MSCore.Functions.Notify('There are not enough police..', 'error', 3500)
                end
            end
        else
            MSCore.Functions.TriggerCallback('MSCore:HasItem', function(result)
                if closestHouse ~= nil then
                    if result then
                        ...
                        MSCore.Functions.Notify('It looks like you lack a toolbox for this...', 'error', 3500)
                    end
                end
            end, "screwdriverset")
        end
...
function lockpickFinish(success)
    if success then
        TriggerServerEvent('ms-houserobbery:server:enterHouse', closestHouse)
        MSCore.Functions.Notify('You got it!', 'success', 2500)
    else
        if usingAdvanced then
            local itemInfo = MSCore.Shared.Items["advancedlockpick"]
        ...
        MSCore.Functions.Notify('Falhaste..', 'error', 2500)
    end
end

RegisterNetEvent('ms-houserobbery:client:setHouseState')
AddEventHandler('ms-houserobbery:client:setHouseState', function(house, state)
...
function searchCabin(cabin)
    ...
    TriggerServerEvent('ms-houserobbery:server:SetBusyState', cabin, currentHouse, true)
    ...
    Skillbar.Start({
        ...
        if SucceededAttempts + 1 >= NeededAttempts then
            ...
            openingDoor = false
            ClearPedTasks(GetPlayerPed(-1))
            TriggerServerEvent('ms-houserobbery:server:searchCabin', cabin, currentHouse)
            Config.Houses[currentHouse]["furniture"][cabin]["searched"] = true
            TriggerServerEvent('ms-houserobbery:server:SetBusyState', cabin, currentHouse, false)
            SucceededAttempts = 0
...
RegisterNetEvent('ms-houserobbery:client:setCabinState')
AddEventHandler('ms-houserobbery:client:setCabinState', function(house, cabin, state)
    Config.Houses[house]["furniture"][cabin]["searched"] = state
end)

RegisterNetEvent('ms-houserobbery:client:SetBusyState')
AddEventHandler('ms-houserobbery:client:SetBusyState', function(cabin, house, bool)
