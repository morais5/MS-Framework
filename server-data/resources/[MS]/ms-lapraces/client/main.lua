...
RegisterNetEvent('ms-lapraces:client:StartRaceEditor')
AddEventHandler('ms-lapraces:client:StartRaceEditor', function(RaceName)
    if not RaceData.InCreator then
        ...
        MSCore.Functions.Notify('Ja estas a fazer uma corrida.', 'error')
    end 
end)
        ...
        while RaceData.InCreator do
            ...
            if PlayerVeh ~= 0 then
                ...
                if IsControlJustPressed(0, Keys["8"]) or IsDisabledControlJustPressed(0, Keys["8"]) then
                    if CreatorData.Checkpoints ~= nil and next(CreatorData.Checkpoints) ~= nil then
                        ...
                        MSCore.Functions.Notify('You have not yet set a checkpoint ..', 'error')
                    end
                end
                ...
                if IsControlJustPressed(0, Keys["K"]) or IsDisabledControlJustPressed(0, Keys["K"]) then
                    if CreatorData.Checkpoints ~= nil and #CreatorData.Checkpoints >= 2 then
                        ...
                        MSCore.Functions.Notify('The minimum of Checkpoints is 10', 'error')
                    end
                end
                ...
                if IsControlJustPressed(0, Keys["]"]) or IsDisabledControlJustPressed(0, Keys["]"]) then
                    if CreatorData.TireDistance + 1.0 ~= 51.0 then
                        ...
                        MSCore.Functions.Notify('You can not do more than 50')
                    end
                end
                ...
                if IsControlJustPressed(0, Keys["["]) or IsDisabledControlJustPressed(0, Keys["["]) then
                    if CreatorData.TireDistance - 1.0 ~= 1.0 then
                        ...
                        MSCore.Functions.Notify('You have to have at least 2')
                    end
                end
            else
            ...
            if IsControlJustPressed(0, Keys["9"]) or IsDisabledControlJustPressed(0, Keys["9"]) then
                if not CreatorData.ConfirmDelete then
                    ...
                    MSCore.Functions.Notify('Click [9] again to confirm', 'error', 5000)
                else
                    ...
                    MSCore.Functions.Notify('Race Edit Canceled!', 'error')
...
function SaveRace()
    ...
    TriggerServerEvent('ms-lapraces:server:SaveRace', CreatorData)

    MSCore.Functions.Notify('Race: '..CreatorData.RaceName..' save!', 'success')
...
function DeleteCheckpoint()
    ...
    if RaceData.ClosestCheckpoint ~= 0 then
        if CreatorData.Checkpoints[RaceData.ClosestCheckpoint] ~= nil then
            ...
            MSCore.Functions.Notify('You can not go so fast...', 'error')
        end
    else
        MSCore.Functions.Notify('You can not go so fast...', 'error')
    end
...
RegisterNetEvent('ms-lapraces:client:UpdateRaceRacerData')
AddEventHandler('ms-lapraces:client:UpdateRaceRacerData', function(RaceId, RaceData)
...
RegisterNetEvent('ms-lapraces:client:JoinRace')
AddEventHandler('ms-lapraces:client:JoinRace', function(Data, Laps)
    if not RaceData.InRace then
        RaceData.InRace = true
        SetupRace(Data, Laps)
        TriggerServerEvent('ms-lapraces:server:UpdateRaceState', CurrentRaceData.RaceId, false, true)
    else
        MSCore.Functions.Notify('Your already in a race..', 'error')
    end
end)

RegisterNetEvent('ms-lapraces:client:LeaveRace')
AddEventHandler('ms-lapraces:client:LeaveRace', function(data)
    MSCore.Functions.Notify('You have left the race!')
    for k, v in pairs(CurrentRaceData.Checkpoints) do
...
RegisterNetEvent('ms-lapraces:client:RaceCountdown')
AddEventHandler('ms-lapraces:client:RaceCountdown', function()
    TriggerServerEvent('ms-lapraces:server:UpdateRaceState', CurrentRaceData.RaceId, true, false)
    if CurrentRaceData.RaceId ~= nil then
        while Countdown ~= 0 do
            if CurrentRaceData.RaceName ~= nil then
                if Countdown == 10 then
                    MSCore.Functions.Notify('The race will start in 10 seconds', 'error', 2500)
                    PlaySound(-1, "slow", "SHORT_PLAYER_SWITCH_SOUND_SET", 0, 0, 1)
                elseif Countdown <= 5 then
                    MSCore.Functions.Notify(Countdown, 'error', 500)
                    PlaySound(-1, "slow", "SHORT_PLAYER_SWITCH_SOUND_SET", 0, 0, 1)
                end
        ...
        if CurrentRaceData.RaceName ~= nil then
            ...
            MSCore.Functions.Notify('GO!', 'success', 1000)
        ...
        MSCore.Functions.Notify('You are currently notin a race..', 'error')
    end
end)
                ...
                if CheckpointDistance < MaxDistance then
                    if CurrentRaceData.TotalLaps == 0 then
                        if CurrentRaceData.CurrentCheckpoint + 1 < #CurrentRaceData.Checkpoints then
                            ...
                            SetNewWaypoint(CurrentRaceData.Checkpoints[CurrentRaceData.CurrentCheckpoint + 1].coords.x, CurrentRaceData.Checkpoints[CurrentRaceData.CurrentCheckpoint + 1].coords.y)
                            TriggerServerEvent('ms-lapraces:server:UpdateRacerData', CurrentRaceData.RaceId, CurrentRaceData.CurrentCheckpoint, CurrentRaceData.Lap, false)
                            DoPilePfx()
                            ...
                            PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
                            CurrentRaceData.CurrentCheckpoint = CurrentRaceData.CurrentCheckpoint + 1
                            TriggerServerEvent('ms-lapraces:server:UpdateRacerData', CurrentRaceData.RaceId, CurrentRaceData.CurrentCheckpoint, CurrentRaceData.Lap, true)
                            FinishRace()
                        end
                    else
                        if CurrentRaceData.CurrentCheckpoint + 1 > #CurrentRaceData.Checkpoints then
...
function FinishRace()
    TriggerServerEvent('ms-lapraces:server:FinishPlayer', CurrentRaceData, CurrentRaceData.TotalTime, CurrentRaceData.TotalLaps, CurrentRaceData.BestLap)
    if CurrentRaceData.BestLap ~= 0 then
        MSCore.Functions.Notify('Race finished '..SecondsToClock(CurrentRaceData.TotalTime)..', Whit best lap as: '..SecondsToClock(CurrentRaceData.BestLap))
    else
        MSCore.Functions.Notify('Race finished '..SecondsToClock(CurrentRaceData.TotalTime))
    end
    for k, v in pairs(CurrentRaceData.Checkpoints) do
        if CurrentRaceData.Checkpoints[k].blip ~= nil then
            RemoveBlip(CurrentRaceData.Checkpoints[k].blip)
...
RegisterNetEvent('ms-lapraces:client:PlayerFinishs')
AddEventHandler('ms-lapraces:client:PlayerFinishs', function(RaceId, Place, FinisherData)
    if CurrentRaceData.RaceId ~= nil then
        if CurrentRaceData.RaceId == RaceId then
            MSCore.Functions.Notify(FinisherData.PlayerData.charinfo.firstname..' Finished on place : '..Place, 'error', 3500)
        end
    end
end)
...
RegisterNetEvent('ms-lapraces:client:WaitingDistanceCheck')
AddEventHandler('ms-lapraces:client:WaitingDistanceCheck', function()
    Wait(1000)
    Citizen.CreateThread(function()
        while true do
                ...
                if CurrentRaceData.Checkpoints[1] ~= nil then
                    ...
                    if dist > 115.0 then
                        if ToFarCountdown ~= 0 then
                            ...
                            MSCore.Functions.Notify('Go back to the start in 10 seconds: '..ToFarCountdown..'s', 'error', 500)
                        else
                            TriggerServerEvent('ms-lapraces:server:LeaveRace', CurrentRaceData)
                            ToFarCountdown = 10
                            break
                        end
                        Citizen.Wait(1000)
