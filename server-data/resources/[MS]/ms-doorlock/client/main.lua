				...
				if ms.Doors[k].pickable then
					if ms.Doors[k].locked then
						if hasItem then
							...
							MSCore.Functions.Notify("A toolkit is missing...", "error")
						end
					else
						MSCore.Functions.Notify('The door is already unlocked ??', 'error', 2500)
					end
				else
					MSCore.Functions.Notify('The door lock is very strong.', 'error', 2500)
				end
			end
		end
    end, "screwdriverset")
...
function lockpickFinish(success)
    if success then
		MSCore.Functions.Notify('Success!', 'success', 2500)
		setDoorLocking(closestDoorValue, closestDoorKey)
    else
        MSCore.Functions.Notify('Failed..', 'error', 2500)
    end
end
