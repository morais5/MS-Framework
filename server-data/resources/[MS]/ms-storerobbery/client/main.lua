...
Citizen.CreateThread(function()
    Wait(1000)
    if MSCore.Functions.GetPlayerData().job ~= nil and next(MSCore.Functions.GetPlayerData().job) then
                            ...
                            if IsControlJustPressed(0, Keys["E"]) then
                                if CurrentCops >= Config.MinimumStoreRobberyPolice then
                                    ...
                                    if math.random(100) <= 50 then
                                        TriggerServerEvent('ms-hud:Server:GainStress', math.random(1, 3))
                                    end
                                    if Config.Safes[safe].type == "keypad" then
                                        ...
                                        MSCore.Functions.TriggerCallback('ms-storerobbery:server:getPadlockCombination', function(combination)
                                    ...
                                    if not copsCalled then
                                        ...
                                        TriggerServerEvent("ms-storerobbery:server:callCops", "safe", currentSafe, streetLabel, pos)
                                        copsCalled = true
                                    end
                                else
                                    MSCore.Functions.Notify("Não existe policia suficiente", "error")
                                end
                            end
                        else
            ...
            if CurrentCops >= Config.MinimumStoreRobberyPolice then
                if usingAdvanced then
                    ...
                    if not copsCalled then
                        ...
                        print('1')
                        TriggerServerEvent("ms-storerobbery:server:callCops", "cashier", currentRegister, streetLabel, pos)
                        copsCalled = true
                    end
                else
                    MSCore.Functions.TriggerCallback('MSCore:HasItem', function(result)
                        if result then
                            ...
                            MSCore.Functions.Notify("You need a toolbox..", "error")
                        end
                    end, "screwdriverset")
                end
...
function setupRegister()
    MSCore.Functions.TriggerCallback('ms-storerobbery:server:getRegisterStatus', function(Registers)
...
function setupSafes()
    MSCore.Functions.TriggerCallback('ms-storerobbery:server:getSafeStatus', function(Safes)
...
RegisterNUICallback('success', function()
    if currentRegister ~= 0 then
        lockpick(false)
        TriggerServerEvent('ms-storerobbery:server:setRegisterStatus', currentRegister)
        local lockpickTime = 30000
        LockpickDoorAnim(lockpickTime)
        MSCore.Functions.Progressbar("search_register", "Emptying the register..", lockpickTime, false, true, {
            ...
            TriggerServerEvent('ms-storerobbery:server:takeMoney', currentRegister, true)            
            currentRegister = 0
        end, function() -- Cancel
            openingDoor = false
            ClearPedTasks(GetPlayerPed(-1))
...
function LockpickDoorAnim(time)
    ...
    Citizen.CreateThread(function()
        while openingDoor do
            ...
            Citizen.Wait(2000)
            time = time - 2
            TriggerServerEvent('ms-storerobbery:server:takeMoney', currentRegister, false)
            if time <= 0 then
                openingDoor = false
        ...
        if won then
            if currentSafe ~= 0 then
                if not Config.Safes[currentSafe].robbed then
                    ...
                    TriggerServerEvent("ms-storerobbery:server:SafeReward", currentSafe)
                    TriggerServerEvent("ms-storerobbery:server:setSafeStatus", currentSafe)
                    currentSafe = 0
                    takeAnim()
                end
            else
...
RegisterNUICallback('fail', function()
    if usingAdvanced then
        if math.random(1, 100) < 20 then
            TriggerServerEvent("MSCore:Server:RemoveItem", "advancedlockpick", 1)
            TriggerEvent('inventory:client:ItemBox', MSCore.Shared.Items["advancedlockpick"], "remove")
        end
    else
        if math.random(1, 100) < 40 then
            TriggerServerEvent("MSCore:Server:RemoveItem", "lockpick", 1)
            TriggerEvent('inventory:client:ItemBox', MSCore.Shared.Items["lockpick"], "remove")
        end
    end
    if (IsWearingHandshoes() and math.random(1, 100) <= 25) then
        local pos = GetEntityCoords(GetPlayerPed(-1))
        TriggerServerEvent("evidence:server:CreateFingerDrop", pos)
        MSCore.Functions.Notify("You cut your hand with the lockpick..")
    end
    lockpick(false)
end)
...
RegisterNUICallback('TryCombination', function(data, cb)
    MSCore.Functions.TriggerCallback('ms-storerobbery:server:isCombinationRight', function(combination)
...
RegisterNetEvent('ms-storerobbery:client:setRegisterStatus')
AddEventHandler('ms-storerobbery:client:setRegisterStatus', function(register, bool)
    Config.Registers[register].robbed = bool
end)

RegisterNetEvent('ms-storerobbery:client:setSafeStatus')
AddEventHandler('ms-storerobbery:client:setSafeStatus', function(safe, bool)
    Config.Safes[safe].robbed = bool
end)

RegisterNetEvent('ms-storerobbery:client:robberyCall')
AddEventHandler('ms-storerobbery:client:robberyCall', function(type, key, streetLabel, coords)
