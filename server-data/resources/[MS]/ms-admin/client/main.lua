...
MSAdmin.Functions.DrawText3D = function(x, y, z, text, lines)
...
AddEventHandler('MSCore:Client:OnPlayerLoaded', function()
    TriggerServerEvent("ms-admin:server:loadPermissions")
end)
...
RegisterNetEvent('ms-admin:client:openMenu')
AddEventHandler('ms-admin:client:openMenu', function(group, dealers)
    ...
    while true do
        ...
        elseif WarMenu.IsMenuOpened('selfOptions') then
            if WarMenu.CheckBox("Noclip", isNoclip, function(checked) isNoclip = checked end) then
                ...
                TriggerServerEvent("ms-admin:server:togglePlayerNoclip", targetId)
            end
            if WarMenu.Button('Revive') then
                local target = PlayerId()
                local targetId = GetPlayerServerId(target)
                TriggerServerEvent('ms-admin:server:revivePlayer', targetId)
            end
        ...
        elseif WarMenu.IsMenuOpened('serverMan') then
            ...
            if WarMenu.ComboBox('Server time', times, currentBanIndex, selectedBanIndex, function(currentIndex, selectedIndex)
                ...
                TriggerServerEvent("ms-weathersync:server:setTime", time.hour, time.minute)
            end
        ...
        elseif WarMenu.IsMenuOpened('playerOptions') then
            if WarMenu.MenuButton('Kill', currentPlayer) then
                TriggerServerEvent("ms-admin:server:killPlayer", GetPlayerServerId(currentPlayer))
            end
            if WarMenu.MenuButton('Revive', currentPlayer) then
                local target = GetPlayerServerId(currentPlayer)
                TriggerServerEvent('ms-admin:server:revivePlayer', target)
            end
            if WarMenu.CheckBox("Noclip", isNoclip, function(checked) isNoclip = checked end) then
                local target = GetPlayerServerId(currentPlayer)
                TriggerServerEvent("ms-admin:server:togglePlayerNoclip", target)
            end
            if WarMenu.CheckBox("Freeze", isFreeze, function(checked) isFreeze = checked end) then
                local target = GetPlayerServerId(currentPlayer)
                TriggerServerEvent("ms-admin:server:Freeze", target, isFreeze)
            end
            ...
            if WarMenu.MenuButton("Give Clothing Menu", currentPlayer) then
                ...
                TriggerServerEvent('ms-admin:server:OpenSkinMenu', targetId)
            end
        ...
        elseif WarMenu.IsMenuOpened('teleportOptions') then
            ...
            if WarMenu.MenuButton('Bring', currentPlayer) then
                ...
                TriggerServerEvent('ms-admin:server:bringTp', GetPlayerServerId(currentPlayer), plyCoords)
            end
            WarMenu.Display()
        elseif WarMenu.IsMenuOpened('permissionOptions') then
            if WarMenu.ComboBox('Permission Group', perms, currentPermIndex, selectedPermIndex, function(currentIndex, selectedIndex)
                ...
                TriggerServerEvent('ms-admin:server:setPermissions', target, group)

                MSCore.Functions.Notify('You changed '..GetPlayerName(currentPlayer)..'\'s group to '..group.label)
            end
            WarMenu.Display()
        elseif WarMenu.IsMenuOpened('adminOptions') then
            if WarMenu.ComboBox('Ban length', bans, currentBanIndex, selectedBanIndex, function(currentIndex, selectedIndex)
                ...
                if reason ~= nil and reason ~= "" and time ~= 0 then
                    local target = GetPlayerServerId(currentPlayer)
                    TriggerServerEvent("ms-admin:server:banPlayer", target, time, reason)
                end
            end
            if WarMenu.MenuButton('Kick', currentPlayer) then
                ...
                if reason ~= nil and reason ~= "" then
                    local target = GetPlayerServerId(currentPlayer)
                    TriggerServerEvent("ms-admin:server:kickPlayer", target, reason)
                end
            end
            WarMenu.Display()
        elseif WarMenu.IsMenuOpened('weatherOptions') then
            for k, v in pairs(AvailableWeatherTypes) do
                if WarMenu.MenuButton(AvailableWeatherTypes[k].label, 'weatherOptions') then
                    TriggerServerEvent('ms-weathersync:server:setWeather', AvailableWeatherTypes[k].weather)
                    MSCore.Functions.Notify('Weather changed to: '..AvailableWeatherTypes[k].label)
                end
            end
        ...
        elseif WarMenu.IsMenuOpened('createDealer') then
            if WarMenu.ComboBox('Hora minima', times, currentMinTimeIndex, selectedMinTimeIndex, function(currentIndex, selectedIndex)
                ...
                MSCore.Functions.Notify('Hora confirmada!', 'success')
            end
    ...
    while true do
        ...
        if showNames then
            for _, player in pairs(GetPlayersFromCoords(GetEntityCoords(GetPlayerPed(-1)), 5.0)) do
                ...
                MSAdmin.Functions.DrawText3D(PlayerCoords.x, PlayerCoords.y, PlayerCoords.z + 1.0, '['..PlayerId..'] '..PlayerName)
            end
        else
            Citizen.Wait(1000)
        end
	...
	while true do
        ...
        if deleteLazer then
            ...
            if hit and (IsEntityAVehicle(entity) or IsEntityAPed(entity) or IsEntityAnObject(entity)) then
                ...
                MSAdmin.Functions.DrawText3D(entityCoord.x, entityCoord.y, entityCoord.z, "Obj: " .. entity .. " Model: " .. GetEntityModel(entity).. " \nPress [~g~E~s~] to delete the object!", 2)
...
RegisterNetEvent('ms-admin:client:bringTp')
AddEventHandler('ms-admin:client:bringTp', function(coords)
    local ped = GetPlayerPed(-1)

    SetEntityCoords(ped, coords.x, coords.y, coords.z)
end)

RegisterNetEvent('ms-admin:client:Freeze')
AddEventHandler('ms-admin:client:Freeze', function(toggle)
    local ped = GetPlayerPed(-1)
...
RegisterNetEvent('ms-admin:client:SendReport')
AddEventHandler('ms-admin:client:SendReport', function(name, src, msg)
    TriggerServerEvent('ms-admin:server:SendReport', name, src, msg)
end)

RegisterNetEvent('ms-admin:client:SendStaffChat')
AddEventHandler('ms-admin:client:SendStaffChat', function(name, msg)
    TriggerServerEvent('ms-admin:server:StaffChatMessage', name, msg)
end)

RegisterNetEvent('ms-admin:client:SaveCar')
AddEventHandler('ms-admin:client:SaveCar', function()
    local ped = GetPlayerPed(-1)
    local veh = GetVehiclePedIsIn(ped)

    if veh ~= nil and veh ~= 0 then
        local plate = GetVehicleNumberPlateText(veh)
        local props = MSCore.Functions.GetVehicleProperties(veh)
        local hash = props.model
        if MSCore.Shared.VehicleModels[hash] ~= nil and next(MSCore.Shared.VehicleModels[hash]) ~= nil then
            TriggerServerEvent('ms-admin:server:SaveCar', props, MSCore.Shared.VehicleModels[hash], GetHashKey(veh), plate)
        else
            MSCore.Functions.Notify('You cant store this vehicle in your garage..', 'error')
        end
    else
...
RegisterNetEvent('ms-admin:client:SetModel')
AddEventHandler('ms-admin:client:SetModel', function(skin)
    local ped = GetPlayerPed(-1)
...
RegisterNetEvent('ms-admin:client:SetSpeed')
AddEventHandler('ms-admin:client:SetSpeed', function(speed)
    local ped = PlayerId()
    if speed == "fast" then
...
RegisterNetEvent('ms-weapons:client:SetWeaponAmmoManual')
AddEventHandler('ms-weapons:client:SetWeaponAmmoManual', function(weapon, ammo)
    local ped = GetPlayerPed(-1)
    if weapon ~= "current" then
        local weapon = weapon:upper()
        SetPedAmmo(ped, GetHashKey(weapon), ammo)
        MSCore.Functions.Notify('+'..ammo..' Ammo for the '..MSCore.Shared.Weapons[GetHashKey(weapon)]["label"], 'success')
    else
        local weapon = GetSelectedPedWeapon(ped)
        if weapon ~= nil then
            SetPedAmmo(ped, weapon, ammo)
            MSCore.Functions.Notify('+'..ammo..' Ammo for the '..MSCore.Shared.Weapons[weapon]["label"], 'success')
        else
            MSCore.Functions.Notify('You dont have a weapon in your hands..', 'error')
        end
    end
end)

RegisterNetEvent('ms-admin:client:GiveNuiFocus')
AddEventHandler('ms-admin:client:GiveNuiFocus', function(focus, mouse)
