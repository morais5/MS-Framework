...
$(document).on('click', '.weapon-attachments-back', function(e){
    e.preventDefault();
    $("#msus-inventory").css({"display":"block"});
    $("#msus-inventory").animate({
        left: 0+"vw"
    }, 200);
...
function FormatAttachmentInfo(data) {
    $.post("http://ms-inventory/GetWeaponData", JSON.stringify({
        weapon: data.name,
...
function handleAttachmentDrag() {
    ...
    $(".weapon-attachments-remove").droppable({
        ...
        drop: function(event, ui) {
            $.post('http://ms-inventory/RemoveAttachment', JSON.stringify({
...
$(document).on('click', '#weapon-attachments', function(e){
    ...
    if (!Inventory.IsWeaponBlocked(ClickedItemData.name)) {
        ...
        $("#msus-inventory").animate({
            ...
            $("#msus-inventory").css({"display":"none"})
        });
        $(".weapon-attachments-container").animate({
            left: 0+"vw"
        }, 200);
        ...
        $.post('http://ms-inventory/Notify', JSON.stringify({
...
function handleDragDrop() {
    ...
    $("#item-use").droppable({
        ...
        drop: function(event, ui) {
            ...
            if(fromData.useable) {
                ...
                $.post("http://ms-inventory/UseItem", JSON.stringify({
    ...
    $("#item-drop").droppable({
        ...
        drop: function(event, ui) {
            ...
            $.post("http://ms-inventory/DropItem", JSON.stringify({
                inventory: fromInventory,
                item: fromData,
...
$(document).on('click', '.CombineItem', function(e){
    ...
    if (combineslotData.toData.combinable.anim != null) {
        $.post('http://ms-inventory/combineWithAnim', JSON.stringify({
        ...
        $.post('http://ms-inventory/combineItem', JSON.stringify({
...
function optionSwitch($fromSlot, $toSlot, $fromInv, $toInv, $toAmount, toData, fromData) {
    ...
    $.post("http://ms-inventory/SetInventoryData", JSON.stringify({
...
function swap($fromSlot, $toSlot, $fromInv, $toInv, $toAmount) {
    ...
    if (fromData !== undefined && fromData.amount >= $toAmount) {       
        ...
        if((toData != undefined || toData != null) && toData.name == fromData.name && !fromData.unique) {
            ...
            $.post("http://ms-inventory/PlayDropSound", JSON.stringify({}));
            $.post("http://ms-inventory/SetInventoryData", JSON.stringify({
                fromInventory: $fromInv.attr("data-inventory"),
                toInventory: $toInv.attr("data-inventory"),
                fromSlot: $fromSlot,
            ...
            if (fromData.amount == $toAmount) {
                if (toData != undefined && toData.combinable != null && isItemAllowed(fromData.name, toData.combinable.accept)) {
                    $.post('http://ms-inventory/getCombineItem', JSON.stringify({item: toData.combinable.reward}), function(item){
...
function InventoryError($elinv, $elslot) {
    ...
    $.post("http://ms-inventory/PlayDropFail", JSON.stringify({}));
}
...
(() => {
    ...
    Inventory.Close = function() {
        ...
        $("#msus-inventory").fadeOut(300);
        $(".combine-option-container").hide();
        $(".item-slot").remove();
        if ($("#rob-money").length) {
            $("#rob-money").remove();
        }
        $.post("http://ms-inventory/CloseInventory", JSON.stringify({}));

        if (AttachmentScreenActive) {
            $("#msus-inventory").css({"left": "0vw"});
...
$(document).on('click', '#rob-money', function(e){
    ...
    $.post('http://ms-inventory/RobMoney', JSON.stringify({
        TargetId: TargetId
    }));
    $("#rob-money").remove();
});