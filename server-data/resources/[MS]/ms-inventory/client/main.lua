...
AddEventHandler('inventory:client:CheckOpenState', function(type, id, label)
    local name = MSCore.Shared.SplitStr(label, "-")[2]
    if type == "stash" then
        ...
        if showTrunkPos and not inInventory then
            ...
            if vehicle ~= 0 and vehicle ~= nil then
                ...
                if (GetDistanceBetweenCoords(pos.x, pos.y, pos.z, vehpos.x, vehpos.y, vehpos.z, true) < 5.0) and not IsPedInAnyVehicle(GetPlayerPed(-1)) then
                    ...
                    MSCore.Functions.DrawText3D(drawpos.x, drawpos.y, drawpos.z, "Bagageira")
...
RegisterCommand("rato", function(source, args, rawCommand)
    ...
    MSCore.Functions.Notify("INFO: Loads [ESC]", "success")
    MSCore.Functions.Notify("Try /rato again and click on [ESC] if you are still buggy", "success")
end)
                        ...
                        if vehicle ~= 0 and vehicle ~= nil then
                            ...
                            if (GetDistanceBetweenCoords(pos.x, pos.y, pos.z, trunkpos) < 2.0) and not IsPedInAnyVehicle(GetPlayerPed(-1)) then
                                if GetVehicleDoorLockStatus(vehicle) < 2 then
                                    ...
                                    MSCore.Functions.Notify("The vehicle is locked..", "error")
                                    return
                                end
                            else
                                CurrentVehicle = nil
    ...
    if bool then
        for k, v in pairs(items) do
            table.insert(itemTable, {
                item = items[k].name,
                label = MSCore.Shared.Items[items[k].name]["label"],
                image = items[k].image,
            })
        end
    end
...
RegisterNUICallback('Notify', function(data, cb)
    MSCore.Functions.Notify(data.message, data.type)
end)
...
AddEventHandler("inventory:client:OpenInventory", function(PlayerAmmo, inventory, other)
    if not IsEntityDead(GetPlayerPed(-1)) then
        ...
        SendNUIMessage({
            ...
            other = other,
            maxweight = MSCore.Config.Player.MaxWeight,
            Ammo = PlayerAmmo,
...
AddEventHandler("inventory:client:UpdatePlayerInventory", function(isError)
    SendNUIMessage({
        action = "update",
        inventory = MSCore.Functions.GetPlayerData().items,
        maxweight = MSCore.Config.Player.MaxWeight,
        slots = MaxInventorySlots,
        error = isError,
    })
...
AddEventHandler("inventory:client:CraftItems", function(itemName, itemCosts, amount, toSlot, points)
    ...
    isCrafting = true
    MSCore.Functions.Progressbar("repair_vehicle", "Craft Successful..", (math.random(2000, 5000) * amount), false, true, {
        ...
        TriggerEvent('inventory:client:ItemBox', MSCore.Shared.Items[itemName], 'add')
        isCrafting = false
	end, function() -- Cancel
		StopAnimTask(GetPlayerPed(-1), "mini@repair", "fixing_a_player", 1.0)
...
AddEventHandler('inventory:client:CraftAttachment', function(itemName, itemCosts, amount, toSlot, points)
    ...
    isCrafting = true
    MSCore.Functions.Progressbar("repair_vehicle", "A craftar arma..", (math.random(2000, 5000) * amount), false, true, {
		disableMovement = true,
		disableCarMovement = true,
		disableMouse = false,
		disableCombat = true,
...
AddEventHandler("inventory:client:PickupSnowballs", function()
    ...
    MSCore.Functions.Progressbar("pickupsnowball", "Picking up balls..", 1500, false, true, {
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        disableCombat = true,
        ...
        TriggerServerEvent('MSCore:Server:AddItem', "snowball", 1)
        TriggerEvent('inventory:client:ItemBox', MSCore.Shared.Items["snowball"], "add")
    end, function() -- Cancel
        ClearPedTasks(GetPlayerPed(-1))
...
AddEventHandler("inventory:client:UseWeapon", function(weaponData, shootbool)
    ...
    elseif weaponName == "weapon_stickybomb" then
        ...
        TriggerServerEvent('MSCore:Server:RemoveItem', weaponName, 1)
    ...
    elseif weaponName == "weapon_snowball" then
        ...
        TriggerEvent('weapons:client:SetCurrentWeapon', weaponData, shootbool)
        MSCore.Functions.TriggerCallback("weapon:server:GetWeaponAmmo", function(result)
...
RegisterNUICallback('GetWeaponData', function(data, cb)
    local data = {
        WeaponData = MSCore.Shared.Items[data.weapon],
...
RegisterNUICallback('RemoveAttachment', function(data, cb)
    local WeaponData = MSCore.Shared.Items[data.WeaponData.name]
    local Attachment = WeaponAttachments[WeaponData.name:upper()][data.AttachmentData.attachment]
    
    MSCore.Functions.TriggerCallback('weapons:server:RemoveAttachment', function(NewAttachments)
...
RegisterNUICallback('getCombineItem', function(data, cb)
    cb(MSCore.Shared.Items[data.item])
end)
...
RegisterNUICallback("combineItem", function(data)
    ...
    TriggerEvent('inventory:client:ItemBox', MSCore.Shared.Items[data.reward], 'add')
end)
...
RegisterNUICallback('combineWithAnim', function(data)
    ...
    MSCore.Functions.Progressbar("combine_anim", animText, animTimeout, false, true, {
        ...
        TriggerEvent('inventory:client:ItemBox', MSCore.Shared.Items[combineData.reward], 'add')
    end, function() -- Cancel
        StopAnimTask(GetPlayerPed(-1), aDict, aLib, 1.0)
...
function ToggleHotbar(toggle)
    local HotbarItems = {
        [1] = MSCore.Functions.GetPlayerData().items[1],
        [2] = MSCore.Functions.GetPlayerData().items[2],
        [3] = MSCore.Functions.GetPlayerData().items[3],
        [4] = MSCore.Functions.GetPlayerData().items[4],
        [5] = MSCore.Functions.GetPlayerData().items[5],
        [41] = MSCore.Functions.GetPlayerData().items[41],
    } 
