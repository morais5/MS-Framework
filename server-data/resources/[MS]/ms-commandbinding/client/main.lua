...
RegisterNetEvent('ms-commandbinding:client:openUI')
AddEventHandler('ms-commandbinding:client:openUI', function()
    openBindingMenu()
end)
        ...
        if isLoggedIn then
            for k, v in pairs(availableKeys) do
                if IsControlJustPressed(0, Keys[v]) or IsDisabledControlJustPressed(0, Keys[v]) then
                    local keyMeta = MSCore.Functions.GetPlayerData().metadata["commandbinds"]
                    local args = {}
                    if next(keyMeta) ~= nil then
                        if keyMeta[v]["command"] ~= "" then
                            ...
                            TriggerServerEvent('MSCore:CallCommand', keyMeta[v]["command"], args)
                            keyPressed = true
                        else
                            MSCore.Functions.Notify('You do not have anything bound to ['..v..'] , /binds for binds a command', 'primary', 4000)
                        end
                    else
                        MSCore.Functions.Notify('Indicates a command, /binds for binde', 'primary', 4000)
                    end
                end
            end
...
RegisterNUICallback('save', function(data)
    ...
    MSCore.Functions.Notify('Saved your binds!', 'success')

    TriggerServerEvent('ms-commandbinding:server:setKeyMeta', keyData)
end)