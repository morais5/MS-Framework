...
AddEventHandler("MSCore:Client:OnPlayerLoaded", function()
    MSCore.Functions.TriggerCallback('ms-pawnshop:melting:server:GetConfig', function(IsMelting, MeltTime, CanTake)
                ...
                if not Config.IsMelting then
                    if Config.CanTake then
                        ...
                        if IsControlJustReleased(0, Keys["E"]) then
                            TriggerServerEvent("ms-pawnshop:server:getGoldBars")
                        end
                    else
                        ...
                        if IsControlJustReleased(0, Keys["E"]) then 
                            ...
                            ScrapAnim(1000)
                            MSCore.Functions.Progressbar("drop_golden_stuff", "Picking up items..", 1000, false, true, {
                                ...
                                if not Config.IsMelting then
                                    ...
                                    TriggerServerEvent("ms-pawnshop:server:meltItems")
                                end
                            end)
                        end
                    end
                ...
                if GetClockHours() >= 9 and GetClockHours() <= 18 then
                    ...
                    elseif sellItemsSet and hasGold then
                        ...
                        if IsControlJustReleased(0, Keys["E"]) then
                            ...
                            MSCore.Functions.Progressbar("sell_gold", "Sell gold..", lockpickTime, false, true, {
                                disableMovement = true,
                                disableCarMovement = true,
                                disableMouse = false,
                                disableCombat = true,
                                ...
                                TriggerServerEvent('ms-pawnshop:server:sellGold')
                            end, function() -- Cancel
                                openingDoor = false
                                ClearPedTasks(GetPlayerPed(-1))
...
function HasPlayerGold()
	local retval = false
	MSCore.Functions.TriggerCallback('ms-pawnshop:server:hasGold', function(result)
		retval = result
	end)
    Citizen.Wait(500)
	return retval
...
RegisterNetEvent('ms-pawnshop:client:startMelting')
AddEventHandler('ms-pawnshop:client:startMelting', function()
    if not Config.IsMelting then
...
RegisterNetEvent('ms-pawnshop:client:SetTakeState')
AddEventHandler('ms-pawnshop:client:SetTakeState', function(state)
