...
$(document).on('click', '#save', function(){
    $.post('http://ms-tunerchip/save', JSON.stringify({
...
$(document).on('click', "#save-neon", function(){
    if (RainbowNeon) {
        $.post('http://ms-tunerchip/saveNeon', JSON.stringify({
...
$(document).on('click', '#save-headlights', function(){
    $.post('http://ms-tunerchip/saveHeadlights', JSON.stringify({
        value: headlightVal,
...
$(document).on('click', '#save-stancer', function(){
    ...
    $.post('http://ms-tunerchip/SetStancer', JSON.stringify({
...
msTuner.Open = function() {
    $.post('http://ms-tunerchip/checkItem', JSON.stringify({item: "tunerlaptop"}), function(hasItem){
        if (hasItem) {
...
msTuner.Close = function() {
    $('.container').fadeOut(250);
    $.post('http://ms-tunerchip/exit');
}