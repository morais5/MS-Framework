...
$(document).on('click', '.bank-app-account', function(e){
    ...
    ms.Phone.Notifications.Add("fas fa-university", "LUNA-Bank", "Copied number!", "#badc58", 1750);
});
...
ms.Phone.Functions.DoBankOpen = function() {
    ms.Phone.Data.PlayerData.money.bank = (ms.Phone.Data.PlayerData.money.bank).toFixed();
    $(".bank-app-account-number").val(ms.Phone.Data.PlayerData.charinfo.account);
    $(".bank-app-account-balance").html("&dollar; "+ms.Phone.Data.PlayerData.money.bank);
    $(".bank-app-account-balance").data('balance', ms.Phone.Data.PlayerData.money.bank);

    $(".bank-app-loaded").css({"display":"none", "padding-left":"30vh"});
    $(".bank-app-accounts").css({"left":"30vh"});
    $(".msank-logo").css({"left": "0vh"});
    $("#msank-text").css({"opacity":"0.0", "left":"9vh"});
...
$(document).on('click', '.bank-app-account-actions', function(e){
    ms.Phone.Animations.TopSlideDown(".bank-app-transfer", 400, 0);
});

$(document).on('click', '#cancel-transfer', function(e){
    e.preventDefault();

    ms.Phone.Animations.TopSlideUp(".bank-app-transfer", 400, -100);
});
    ...
    if (iban != "" && amount != "") {
            $.post('http://ms-phone/CanTransferMoney', JSON.stringify({
                ...
                if (data.TransferedMoney) {
                    ...
                    $(".bank-app-account-balance").html("&dollar; " + (data.NewBalance).toFixed(0));
                    $(".bank-app-account-balance").data('balance', (data.NewBalance).toFixed(0));
                    ms.Phone.Notifications.Add("fas fa-university", "LUNA-Bank", "Transfer &dollar; "+amount+",-!", "#badc58", 1500);
                } else {
                    ms.Phone.Notifications.Add("fas fa-university", "LUNA-Bank", "You do not have this amount of money!", "#badc58", 1500);
                }
                ms.Phone.Animations.TopSlideUp(".bank-app-transfer", 400, -100);
            });
    } else {
        ms.Phone.Notifications.Add("fas fa-university", "LUNA-Bank", "Fill all the fields", "#badc58", 1750);
    }
});
    ...
    if (BankBalance >= InvoiceData.amount) {
        $.post('http://ms-phone/PayInvoice', JSON.stringify({
            ...
            if (CanPay) {
                ...
                ms.Phone.Notifications.Add("fas fa-university", "LUNA-Bank", "You have paid &dollar;"+InvoiceData.amount+"!", "#badc58", 1500);
                var amountData = $(".bank-app-account-balance").data('balance');
...
$(document).on('click', '.decline-invoice', function(event){
    ...
    /*$("#"+InvoiceId).animate({//EU COMENTEI ISTO
        left: 30+"vh",
    }, 300, function(){
        setTimeout(function(){
            $("#"+InvoiceId).remove();
    ...
    ms.Phone.Notifications.Add("fas fa-university", "OFFS-Bank", "Pagaste &dollar;"+InvoiceData.amount+"!", "#badc58", 1500);*/
});

ms.Phone.Functions.LoadBankInvoices = function(invoices) {
    if (invoices !== null) {
...
ms.Phone.Functions.LoadContactsWithNumber = function(myContacts) {
    ...
    if (myContacts !== null) {
        $.each(myContacts, function(i, contact){
            ...
            var ContactColor = ms.Phone.ContactColors[RandomNumber];
...
$(document).on('click', '.bank-app-my-contacts-list-back', function(e){
    ...
    ms.Phone.Animations.TopSlideUp(".bank-app-my-contacts", 400, -100);
});

$(document).on('click', '.bank-transfer-mycontacts-icon', function(e){
    e.preventDefault();

    ms.Phone.Animations.TopSlideDown(".bank-app-my-contacts", 400, 0);
});
...
$(document).on('click', '.bank-app-my-contact', function(e){
    ...
    if (PressedContactData.iban !== "" && PressedContactData.iban !== undefined && PressedContactData.iban !== null) {
        ...
        ms.Phone.Notifications.Add("fas fa-university", "LUNA-Bank", "There is no bank account linked to this number!", "#badc58", 2500);
    }
    ms.Phone.Animations.TopSlideUp(".bank-app-my-contacts", 400, -100);
});