...
Citizen.CreateThread(function()
    ...
    local requiredItems3 = {
        [1] = {name = MSCore.Shared.Items["thermite"]["name"], image = MSCore.Shared.Items["thermite"]["image"]},
    }
    ...
    local requiredItems = {
        [1] = {name = MSCore.Shared.Items["security_card_02"]["name"], image = MSCore.Shared.Items["security_card_02"]["image"]},
    }
    while true do
                    ...
                    if CurrentCops >= Config.MinimumPacificPolice then
                        if not Config.BigBanks["pacific"]["isOpened"] then 
                            MSCore.Functions.TriggerCallback('MSCore:HasItem', function(result)
                                if result then 
                                    TriggerEvent('inventory:client:requiredItems', requiredItems, false)
                                    MSCore.Functions.Progressbar("hack_gate", "Connecting electronic kit..", math.random(5000, 10000), false, true, {
                                        ...
                                        if not copsCalled then
                                            ...
                                            if Config.BigBanks["pacific"]["alarm"] then
                                                TriggerServerEvent("ms-bankrobbery:server:callCops", "pacific", 0, streetLabel, pos)
                                                copsCalled = true
                                            end
                                        end
                                    end, function() -- Cancel
...
RegisterNetEvent('ms-bankrobbery:UseBankcardB')
AddEventHandler('ms-bankrobbery:UseBankcardB', function()
    local ped = GetPlayerPed(-1)
    local pos = GetEntityCoords(ped)
        ...
        MSCore.Functions.TriggerCallback('ms-bankrobbery:server:isRobberyActive', function(isBusy)
            if not isBusy then
                if CurrentCops >= Config.MinimumPacificPolice then
                    if not Config.BigBanks["pacific"]["isOpened"] then 
                        TriggerEvent('inventory:client:requiredItems', requiredItems2, false)
                        MSCore.Functions.Progressbar("security_pass", "Typing the password..", math.random(5000, 10000), false, true, {
                            disableMovement = true,
                            disableCarMovement = true,
                            disableMouse = false,
                            disableCombat = true,
                            ...
                            StopAnimTask(GetPlayerPed(-1), "anim@gangops@facility@servers@", "hotwire", 1.0)
                            TriggerServerEvent('ms-doorlock:server:updateState', 76, false)
                            TriggerServerEvent("MSCore:Server:RemoveItem", "security_card_02", 1)
                            if not copsCalled then
                                local s1, s2 = Citizen.InvokeNative(0x2EB41072B4C1E4C0, pos.x, pos.y, pos.z, Citizen.PointerValueInt(), Citizen.PointerValueInt())
                                local street1 = GetStreetNameFromHashKey(s1)
                                local street2 = GetStreetNameFromHashKey(s2)
...
function OnHackPacificDone(success, timeremaining)
    if success then
        ...
        TriggerServerEvent('ms-bankrobbery:server:setBankState', "pacific", true)
    else
		TriggerEvent('mhacking:hide')
	end
end
