        ...
        if dist < 1.5 then
            if CurrentCops >= Config.MinimumThermitePolice then
                if not Config.PowerStations[closestStation].hit then
                    ...
                    MSCore.Functions.Notify("It seems that the fuses burned..", "error")
                end
            else
...
RegisterNetEvent('ms-bankrobbery:client:SetStationStatus')
AddEventHandler('ms-bankrobbery:client:SetStationStatus', function(key, isHit)
...
RegisterNUICallback('thermitefailed', function()
    ...
    TriggerServerEvent("MSCore:Server:RemoveItem", "thermite", 1)
    TriggerEvent('inventory:client:ItemBox', MSCore.Shared.Items["thermite"], "remove")
...
RegisterNUICallback('thermitesuccess', function()
    ...
    local coords = GetEntityCoords(GetPlayerPed(-1))
    while time > 0 do 
        MSCore.Functions.Notify("Will start in " .. time .. "..")
        Citizen.Wait(1000)
        time = time - 1
    end
    local randTime = math.random(10000, 15000)
    ...
    if currentStation ~= 0 then
        MSCore.Functions.Notify("The fuses have been broken", "success")
        TriggerServerEvent("ms-bankrobbery:server:SetStationStatus", currentStation, true)
    elseif currentGate ~= 0 then
        MSCore.Functions.Notify("The door is now open", "success")
        TriggerServerEvent('ms-doorlock:server:updateState', currentGate, false)
        currentGate = 0
    end
end)
