...
Citizen.CreateThread(function()
    Citizen.Wait(2000)
    local requiredItems = {
        [1] = {name = MSCore.Shared.Items["security_card_01"]["name"], image = MSCore.Shared.Items["security_card_01"]["image"]},
    }
                                    ...
                                    if lockerDist < 0.5 then
                                        ...
                                        if IsControlJustPressed(0, Keys["E"]) then
                                            if CurrentCops >= Config.MinimumPaletoPolice then
                                                ...
                                                MSCore.Functions.Notify("Não existe policias suficientes.. ", "error")
                                            end
                                        end
                                    end
                                end
...
RegisterNetEvent('ms-bankrobbery:UseBankcardA')
AddEventHandler('ms-bankrobbery:UseBankcardA', function()
    local ped = GetPlayerPed(-1)
    local pos = GetEntityCoords(ped)
    local dist = GetDistanceBetweenCoords(pos, Config.BigBanks["paleto"]["coords"]["x"], Config.BigBanks["paleto"]["coords"]["y"],Config.BigBanks["paleto"]["coords"]["z"])
        ...
        MSCore.Functions.TriggerCallback('ms-bankrobbery:server:isRobberyActive', function(isBusy)
            if not isBusy then
                if CurrentCops >= Config.MinimumPaletoPolice then
                    if not Config.BigBanks["paleto"]["isOpened"] then 
                        TriggerEvent('inventory:client:requiredItems', requiredItems, false)
                        MSCore.Functions.Progressbar("security_pass", "Validitating card..", math.random(5000, 10000), false, true, {
                            ...
                            TriggerServerEvent('ms-bankrobbery:server:setBankState', "paleto", true)
                            TriggerServerEvent("MSCore:Server:RemoveItem", "security_card_01", 1)
                            TriggerServerEvent('ms-doorlock:server:updateState', 85, false)
                            if not copsCalled then
                                ...
                                if Config.BigBanks["paleto"]["alarm"] then
                                    TriggerServerEvent("ms-bankrobbery:server:callCops", "paleto", 0, streetLabel, pos)
                                    copsCalled = true
                                end
                            end
                        end, function() -- Cancel
