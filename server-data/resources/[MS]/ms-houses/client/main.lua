...
RegisterNetEvent('ms-houses:client:sellHouse')
AddEventHandler('ms-houses:client:sellHouse', function()
    if closesthouse ~= nil and hasKey then
        TriggerServerEvent('ms-houses:server:viewHouse', closesthouse)
    end
end)
...
RegisterNetEvent('ms-houses:client:EnterHouse')
AddEventHandler('ms-houses:client:EnterHouse', function()
...
RegisterNetEvent('ms-houses:client:RequestRing')
AddEventHandler('ms-houses:client:RequestRing', function()
    local ped = GetPlayerPed(-1)
    local pos = GetEntityCoords(ped)

    if closesthouse ~= nil then
        TriggerServerEvent('ms-houses:server:RingDoor', closesthouse)
    end
end)
...
Citizen.CreateThread(function()
    ...
    TriggerServerEvent('ms-houses:client:setHouses')
    isLoggedIn = true
    SetClosestHouse()
    TriggerEvent('ms-houses:client:setupHouseBlips')
    Citizen.Wait(100)
    TriggerEvent('ms-garages:client:setHouseGarage', closesthouse, hasKey)
    TriggerServerEvent("ms-houses:server:setHouses")
end)
...
AddEventHandler('ms-houses:client:setHouseConfig', function(houseConfig)
    ...
    --TriggerEvent("ms-houses:client:refreshHouse")
end)

RegisterNetEvent('ms-houses:client:lockHouse')
AddEventHandler('ms-houses:client:lockHouse', function(bool, house)
    Config.Houses[house].locked = bool
end)

RegisterNetEvent('ms-houses:client:createHouses')
AddEventHandler('ms-houses:client:createHouses', function(price, tier)
    ...
    street = street:gsub("%-", " ")
    TriggerServerEvent('ms-houses:server:addNewHouse', street, coords, price, tier)
end)

RegisterNetEvent('ms-houses:client:addGarage')
AddEventHandler('ms-houses:client:addGarage', function()
    if closesthouse ~= nil then 
        local pos = GetEntityCoords(GetPlayerPed(-1))
        local heading = GetEntityHeading(GetPlayerPed(-1))
        local coords = {
        ...
        TriggerServerEvent('ms-houses:server:addGarage', closesthouse, coords)
    else
        MSCore.Functions.Notify("No house around..", "error")
    end
end)

RegisterNetEvent('ms-houses:client:toggleDoorlock')
AddEventHandler('ms-houses:client:toggleDoorlock', function()
    local ped = GetPlayerPed(-1)
    local pos = GetEntityCoords(ped)
    ...
    if(GetDistanceBetweenCoords(pos, Config.Houses[closesthouse].coords.enter.x, Config.Houses[closesthouse].coords.enter.y, Config.Houses[closesthouse].coords.enter.z, true) < 1.5)then
        if hasKey then
            if Config.Houses[closesthouse].locked then
                TriggerServerEvent('ms-houses:server:lockHouse', false, closesthouse)
                MSCore.Functions.Notify("House is unlocked!", "success", 2500)
            else
                TriggerServerEvent('ms-houses:server:lockHouse', true, closesthouse)
                MSCore.Functions.Notify("House is locked!", "error", 2500)
            end
        else
            MSCore.Functions.Notify("You dont have the keys of the house...", "error", 3500)
        end
    else
        MSCore.Functions.Notify("There is no door to see??", "error", 3500)
    end
end)
                    ...
                    if CurrentDoorBell ~= 0 then
                        if(GetDistanceBetweenCoords(pos, Config.Houses[closesthouse].coords.enter.x + POIOffsets.exit.x, Config.Houses[closesthouse].coords.enter.y + POIOffsets.exit.y, Config.Houses[closesthouse].coords.enter.z - Config.MinZOffset + POIOffsets.exit.z, true) < 1.5)then
                            ...
                            if IsControlJustPressed(0, Keys["G"]) then
                                TriggerServerEvent("ms-houses:server:OpenDoor", CurrentDoorBell, closesthouse)
                                CurrentDoorBell = 0
                            end
                        end
                    end
                    ...
                    if CurrentHouse ~= nil then
                        if outfitLocation ~= nil then
                            if(GetDistanceBetweenCoords(pos, outfitLocation.x, outfitLocation.y, outfitLocation.z, true) < 1.5)then
                                ...
                                if IsControlJustPressed(0, Keys["E"]) then
                                    TriggerEvent('ms-clothing:client:openOutfitMenu')
                                end
                        ...
                        if logoutLocation ~= nil then
                            if(GetDistanceBetweenCoords(pos, logoutLocation.x, logoutLocation.y, logoutLocation.z, true) < 1.5)then
                                ...
                                if IsControlJustPressed(0, Keys["E"]) then
                                    ...
                                    exports['ms-interior']:DespawnInterior(houseObj, function()
                                        TriggerEvent('ms-weathersync:client:EnableSync')
                                        ...
                                        inOwned = false
                                        inside = false
                                        TriggerServerEvent('ms-houses:server:LogoutLocation')
                                    end)
                                end
...
RegisterNetEvent('ms-houses:client:RingDoor')
AddEventHandler('ms-houses:client:RingDoor', function(player, house)
    if closesthouse == house and inside then
        CurrentDoorBell = player
        TriggerServerEvent("InteractSound_SV:PlayOnSource", "doorbell", 0.1)
        MSCore.Functions.Notify("Some one is ringing the door!")
    end
end)
...
RegisterNetEvent('ms-houses:client:giveHouseKey')
AddEventHandler('ms-houses:client:giveHouseKey', function(data)
    ...
    if player ~= -1 and distance < 2.5 and closesthouse ~= nil then
        ...
        if housedist < 10 then
            TriggerServerEvent('ms-houses:server:giveHouseKey', playerId, closesthouse)
        else
            MSCore.Functions.Notify("You'r not close enough to the door..", "error")
        end
    elseif closesthouse == nil then
        MSCore.Functions.Notify("There is no house near you", "error")
    else
        MSCore.Functions.Notify("No one around!", "error")
    end
end)

RegisterNetEvent('ms-houses:client:removeHouseKey')
AddEventHandler('ms-houses:client:removeHouseKey', function(data)
    if closesthouse ~= nil then 
        local housedist = GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), Config.Houses[closesthouse].coords.enter.x, Config.Houses[closesthouse].coords.enter.y, Config.Houses[closesthouse].coords.enter.z)
        if housedist < 5 then
            MSCore.Functions.TriggerCallback('ms-houses:server:getHouseOwner', function(result)
                if MSCore.Functions.GetPlayerData().citizenid == result then
                    inHoldersMenu = true
                    HouseKeysMenu()
                    Menu.hidden = not Menu.hidden
                else
                    MSCore.Functions.Notify("You're not a house owner..", "error")
                end
            end, closesthouse)
        else
            MSCore.Functions.Notify("You'r not close enough to the door..", "error")
...
RegisterNetEvent('ms-houses:client:refreshHouse')
AddEventHandler('ms-houses:client:refreshHouse', function(data)
    Citizen.Wait(100)
    SetClosestHouse()
    --TriggerEvent('ms-garages:client:setHouseGarage', closesthouse, hasKey)
end)

RegisterNetEvent('ms-houses:client:SpawnInApartment')
AddEventHandler('ms-houses:client:SpawnInApartment', function(house)
    local pos = GetEntityCoords(GetPlayerPed(-1))
...
function HouseKeysMenu()
    ...
    MenuTitle = "Sleutels"
    ClearMenu()
    MSCore.Functions.TriggerCallback('ms-houses:server:getHouseKeyHolders', function(holders)
        ped = GetPlayerPed(-1);
        MenuTitle = "Sleutelhouders:"
        ClearMenu()
        if holders == nil or next(holders) == nil then
            MSCore.Functions.Notify("No key holders found..", "error", 3500)
            closeMenuFull()
        else
...
function removeHouseKey(citizenData)
    TriggerServerEvent('ms-houses:server:removeHouseKey', closesthouse, citizenData)
    closeMenuFull()
end

function removeOutfit(oData)
    TriggerServerEvent('clothes:removeOutfit', oData.outfitname)
    MSCore.Functions.Notify(oData.outfitname.." Is removed", "success", 2500)
    closeMenuFull()
end
...
function enterOwnedHouse(house)
    ...
    if Config.Houses[house].tier == 1 then
        data = exports['ms-interior']:CreateTier1House(coords)
    elseif Config.Houses[house].tier == 2 then
        data = exports['ms-interior']:CreateTrevorsShell(coords)
    elseif Config.Houses[house].tier == 3 then
        data = exports['ms-interior']:CreateMichaelShell(coords)
    elseif Config.Houses[house].tier == 4 then
        data = exports['ms-interior']:CreateApartmentShell(coords)
    elseif Config.Houses[house].tier == 5 then
        data = exports['ms-interior']:CreateCaravanShell(coords)
    elseif Config.Houses[house].tier == 6 then
        data = exports['ms-interior']:CreateFranklinShell(coords)
    elseif Config.Houses[house].tier == 7 then
        data = exports['ms-interior']:CreateFranklinAuntShell(coords)
    end
    ...
    entering = true
    TriggerServerEvent('ms-houses:server:SetInsideMeta', house, true)
    Citizen.Wait(500)
    SetRainFxIntensity(0.0)
    TriggerEvent('ms-weathersync:client:DisableSync')
    TriggerEvent('ms-weed:client:getHousePlants', house)
    Citizen.Wait(100)
...
RegisterNetEvent('ms-houses:client:enterOwnedHouse')
AddEventHandler('ms-houses:client:enterOwnedHouse', function(house)
...
RegisterNUICallback('HasEnoughMoney', function(data, cb)
    MSCore.Functions.TriggerCallback('ms-houses:server:HasEnoughMoney', function(hasEnough)
        
    end, data.objectData)
end)

RegisterNetEvent('ms-houses:client:LastLocationHouse')
AddEventHandler('ms-houses:client:LastLocationHouse', function(houseId)
    MSCore.Functions.GetPlayerData(function(PlayerData)
		if PlayerData.metadata["injail"] == 0 then
...
function leaveOwnedHouse(house)
    if not FrontCam then
        ...
        exports['ms-interior']:DespawnInterior(houseObj, function()
            ...
            SetEntityHeading(GetPlayerPed(-1), Config.Houses[CurrentHouse].coords.enter.h)
            TriggerEvent('ms-weed:client:leaveHouse')
            TriggerServerEvent('ms-houses:server:SetInsideMeta', house, false)
            CurrentHouse = nil
        end)
    end
end
...
RegisterNetEvent('ms-houses:client:setupHouseBlips')
AddEventHandler('ms-houses:client:setupHouseBlips', function()
    Citizen.CreateThread(function()
        Citizen.Wait(2000)
        if isLoggedIn then
            MSCore.Functions.TriggerCallback('ms-houses:server:getOwnedHouses', function(ownedHouses)
...
RegisterNetEvent('ms-houses:client:SetClosestHouse')
AddEventHandler('ms-houses:client:SetClosestHouse', function()
    SetClosestHouse()
end)
...
RegisterNUICallback('buy', function()
    openContract(false)
    disableViewCam()
    TriggerServerEvent('ms-houses:server:buyHouse', closesthouse)
end)
...
RegisterNetEvent('ms-houses:client:viewHouse')
AddEventHandler('ms-houses:client:viewHouse', function(houseprice, brokerfee, bankfee, taxes, firstname, lastname)
...
function SetClosestHouse()
    ...
    if not inside then
        ...
        if closesthouse ~= nil then 
            MSCore.Functions.TriggerCallback('ms-houses:server:hasKey', function(result)
                hasKey = result
            end, closesthouse)
        
            MSCore.Functions.TriggerCallback('ms-houses:server:isOwned', function(result)
                isOwned = result
            end, closesthouse)
        end
    end
...
function setHouseLocations()
    if closesthouse ~= nil then
        MSCore.Functions.TriggerCallback('ms-houses:server:getHouseLocations', function(result)
            if result ~= nil then
...
RegisterNetEvent('ms-houses:client:setLocation')
AddEventHandler('ms-houses:client:setLocation', function(data)
    local ped = GetPlayerPed(-1)
    local pos = GetEntityCoords(ped)
    ...
    if inside then
        if hasKey then
            if data.id == 'setstash' then
                TriggerServerEvent('ms-houses:server:setLocation', coords, closesthouse, 1)
            elseif data.id == 'setoutift' then
                TriggerServerEvent('ms-houses:server:setLocation', coords, closesthouse, 2)
            elseif data.id == 'setlogout' then
                TriggerServerEvent('ms-houses:server:setLocation', coords, closesthouse, 3)
            end
        else
            MSCore.Functions.Notify('Je bent niet de eigenaar van het huis..', 'error')
        end
    else    
        MSCore.Functions.Notify('You are not in a house..', 'error')
    end
end)

RegisterNetEvent('ms-houses:client:refreshLocations')
AddEventHandler('ms-houses:client:refreshLocations', function(house, location, type)
    local ped = GetPlayerPed(-1)
    local pos = GetEntityCoords(ped)
...
RegisterNetEvent('ms-houses:client:HomeInvasion')
AddEventHandler('ms-houses:client:HomeInvasion', function()
    local ped = GetPlayerPed(-1)
    local pos = GetEntityCoords(ped)
    local Skillbar = exports['ms-skillbar']:GetSkillbarObject()

    if closesthouse ~= nil then
        MSCore.Functions.TriggerCallback('police:server:IsPoliceForcePresent', function(IsPresent)
            if IsPresent then
                local dist = GetDistanceBetweenCoords(pos, Config.Houses[closesthouse].coords.enter.x, Config.Houses[closesthouse].coords.enter.y, Config.Houses[closesthouse].coords.enter.z, true)
                        ...
                        if not Config.Houses[closesthouse].IsRaming then
                            ...
                            Skillbar.Start({
                                ...
                                if RamsDone + 1 >= Config.RamsNeeded then
                                    TriggerServerEvent('ms-houses:server:lockHouse', false, closesthouse)
                                    MSCore.Functions.Notify('It worked the door is now out.', 'success')
                                    TriggerServerEvent('ms-houses:server:SetHouseRammed', true, closesthouse)
                                ...
                                RamsDone = 0
                                TriggerServerEvent('ms-houses:server:SetRamState', false, closesthouse)
                                MSCore.Functions.Notify('It faild try again.', 'error')
                                DoRamAnimation(false)
                            end)
                            TriggerServerEvent('ms-houses:server:SetRamState', true, closesthouse)
                        else
                            MSCore.Functions.Notify('Er is al iemand bezig met de deur..', 'error')
                        end
                    else
                        MSCore.Functions.Notify('19/5000 This house is already open..', 'error')
                    end
                else
                    MSCore.Functions.Notify('You\'re not near a house..', 'error')
                end
            else
                MSCore.Functions.Notify('There is no police force present..', 'error')
            end
        end)
    else
        MSCore.Functions.Notify('You\'re not near a house..', 'error')
...
RegisterNetEvent('ms-houses:client:SetRamState')
AddEventHandler('ms-houses:client:SetRamState', function(bool, house)
    Config.Houses[house].IsRaming = bool
end)

RegisterNetEvent('ms-houses:client:SetHouseRammed')
AddEventHandler('ms-houses:client:SetHouseRammed', function(bool, house)
    Config.Houses[house].IsRammed = bool
end)

RegisterNetEvent('ms-houses:client:ResetHouse')
AddEventHandler('ms-houses:client:ResetHouse', function()
    local ped = GetPlayerPed(-1)

    if closesthouse ~= nil then
        if Config.Houses[closesthouse].IsRammed == nil then
            Config.Houses[closesthouse].IsRammed = false
            TriggerServerEvent('ms-houses:server:SetHouseRammed', false, closesthouse)
            TriggerServerEvent('ms-houses:server:SetRamState', false, closesthouse)
        end
        if Config.Houses[closesthouse].IsRammed then
            ...
            TriggerServerEvent('ms-houses:server:lockHouse', true, closesthouse)
            RamsDone = 0
            MSCore.Functions.Notify('You locked the house again..', 'success')
        else
            MSCore.Functions.Notify('This door is not broken open  ..', 'error')
        end
    end
end)