...
$('document').ready(function() {
    ...
    window.addEventListener('message', function(event) {
        ...
        if (item.type == "buyOption") {
            ...
            $(".decorate-confirm").find("p").html("Are you sure you want to buy the object for $"+selectedObjectData.price+"?");
        }
    ...
    document.onkeyup = function (data) {
        if (data.which == 116 ) {
            $.post('http://ms-houses/toggleCursor');
        }

        if (data.which == 13 ) {
            if (selectedObjectData != null) {
                $.post('http://ms-houses/editOwnedObject', JSON.stringify({
                    objectData: selectedObjectData
                }));
                selectedObjectData = null;
                $.post('http://ms-houses/setupMyObjects', JSON.stringify({}), function(myObjects){
                    $('.decorate-items').html("");
                ...
                if (objData != null) {
                    $.post("http://ms-houses/spawnobject", JSON.stringify({
...
$(document).on('click', '.header-btn', function(){
    ...
    $.post('http://ms-houses/ResetSelectedProp');
...
$(document).on('click', '.footer-btn', function(){
    ...
    if (selectedCategory != "remove-owned-obj") {
        ...
        $(".decorate-footer-buttons").html("");
        $(selectedHeaderButton).removeClass('header-btn-selected');
        $.post('http://ms-houses/deleteSelectedObject');
        $(".decorate-footer-buttons").fadeOut(150);
        $(".decorate-items").fadeOut(150);
    }
});
...
$(document).on('click', '#buy-object', function(){
    $.post('http://ms-houses/buySelectedObject');
    selectedObjectData = null;
    $(".decorate-confirm").css("display", "none");
});

$(document).on('click', '#cancel-object', function(){
    $.post('http://ms-houses/cancelSelectedObject');
    selectedObjectData = null;
    $(".decorate-confirm").css("display", "none");
});
...
$(document).on('click', '.decorate-item', function(){
    ...
    if ($("#"+objId).data('type') == "newObject") {
        if (selectedObject == this) {
            ...
            selectedObject = null;
            $.post('http://ms-houses/removeObject');
        } else {
            $(this).addClass('selected-object');
            selectedObject = this;
            $.post("http://ms-houses/chooseobject", JSON.stringify({
                object: objData.object,
            }));
        ...
        if (selectedObject == this) {
            $(this).removeClass('selected-object');
            $.post('http://ms-houses/deselectOwnedObject')
            selectedObject = null;
        } else {
            $(this).addClass('selected-object');
            selectedObject = this;
            selectedObjectData = myObjectData;
            $.post('http://ms-houses/selectOwnedObject', JSON.stringify({
...
Decorations.Close = function() {
    ...
    selectedObjectData = null;
    $.post("http://ms-houses/closedecorations", JSON.stringify({}));
}

$(".property-accept").click(function(e){
    $.post('http://ms-houses/buy', JSON.stringify({}))
});

$(".property-cancel").click(function(e){
    $.post('http://ms-houses/exit', JSON.stringify({}));
});