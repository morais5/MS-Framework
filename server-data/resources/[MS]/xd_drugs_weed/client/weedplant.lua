	...
	while true do
		...
		if nearbyObject and IsPedOnFoot(playerPed) then
			...
			if not isPickingUp then
				MSCore.Functions.Draw2DText(0.5, 0.88, 'Press ~g~[E]~w~ to pick up', 0.5)
			end
			...
			if IsControlJustReleased(0, 38) and not isPickingUp then
				...
				MSCore.Functions.Progressbar("search_register", "Picking up..", 7500, false, true, {
					...
					MSCore.Functions.DeleteObject(nearbyObject)
...
AddEventHandler('onResourceStop', function(resource) --weedPlants
	if resource == GetCurrentResourceName() then
		for k, v in pairs(weedPlants) do
			MSCore.Functions.DeleteObject(v)
		end
	end
end)
function SpawnWeedPlants() --This spawns in the Weed plants, 
	while spawnedWeed < 25 do
		...
		MSCore.Functions.SpawnLocalObject('prop_weed_02', weedCoords, function(obj) --- change this prop to whatever plant you are trying to use 
	...
	while true do
		...
		if GetDistanceBetweenCoords(coords, Config.CircleZones.WeedProcessing.coords, true) < 1 then
			...
			if not isProcessing then
				MSCore.Functions.DrawText3D(Config.CircleZones.WeedProcessing.coords.x, Config.CircleZones.WeedProcessing.coords.y, Config.CircleZones.WeedProcessing.coords.z, 'Carrega ~g~[ E ]~w~ para processar')
			end
			...
			if IsControlJustReleased(0, 38) and not isProcessing then
				...
				if (hasWeed and hasBag) then
					Processweed()
				elseif (hasWeed) then
					MSCore.Functions.Notify('You dont have enough plastic bags.', 'error')
				elseif (hasBag) then
					MSCore.Functions.Notify('You dont have enough cannabis.', 'error')
				else
					MSCore.Functions.Notify('You dont have enough cannabis or plastic bags.', 'error')
				end
			end
		else
			Citizen.Wait(500)
...
function Processweed()  -- simple animations to loop while process is taking place
	...
	MSCore.Functions.Progressbar("search_register", "Trying to process..", 15000, false, true, {
		disableMovement = true,
		disableCarMovement = true,
		disableMouse = false,
		disableCombat = true,
	...
	while true do
		...
		if GetDistanceBetweenCoords(coords, Config.CircleZones.DrugDealer.coords, true) < 1 then
			...
			if not isProcessing2 then
				MSCore.Functions.DrawText3D(Config.CircleZones.DrugDealer.coords.x, Config.CircleZones.DrugDealer.coords.y, Config.CircleZones.DrugDealer.coords.z, 'Press ~g~[ E ]~w~ to sell')
			end
