...
RegisterNetEvent('ms-drugs:client:cornerselling')
AddEventHandler('ms-drugs:client:cornerselling', function(data)
    MSCore.Functions.TriggerCallback('ms-drugs:server:cornerselling:getAvailableDrugs', function(result)
        if result ~= nil then
            availableDrugs = result

            if not cornerselling then
                cornerselling = true
                MSCore.Functions.Notify('You are now selling drugs...')
                startLocation = GetEntityCoords(GetPlayerPed(-1))
                -- TaskStartScenarioInPlace(GetPlayerPed(-1), "CODE_HUMAN_CROSS_ROAD_WAIT", 0, false)
            else
                cornerselling = false
                MSCore.Functions.Notify('You stopped selling drugs...')
                -- ClearPedTasks(GetPlayerPed(-1))
            end
        else
            MSCore.Functions.Notify('You dont have drugs with you...', 'error')
        end
    end)
end)

function toFarAway()
    MSCore.Functions.Notify('Leave the place, start over!', 'error')
    cornerselling = false
    hasTarget = false
    busySelling = false
    startLocation = nil
            ...
            if IsEntityDead(stealingPed) then
                ...
                if (GetDistanceBetweenCoords(pos.x, pos.y, pos.z, pedpos.x, pedpos.y, pedpos.z, true) < 1.5) then
                    ...
                    if IsControlJustReleased(0, Keys["E"]) then
                        ...
                        Citizen.Wait(2000)
                        ClearPedTasks(GetPlayerPed(-1))
                        TriggerServerEvent("MSCore:Server:AddItem", stealData.item, stealData.amount)
                        TriggerEvent('inventory:client:ItemBox', MSCore.Shared.Items[stealData.item], "add")
                        stealingPed = nil
                        stealData = {}
                    end
                end
...
RegisterNetEvent('ms-drugs:client:refreshAvailableDrugs')
AddEventHandler('ms-drugs:client:refreshAvailableDrugs', function(items)
...
function SellToPed(ped)
    ...
    if hasTarget then
        while pedDist < 1.5 do
            ...
            if getRobbed == 18 or getRobbed == 9 then
                TriggerServerEvent('ms-drugs:server:robCornerDrugs', availableDrugs[drugType].item, bagAmount)
                MSCore.Functions.Notify('You were robbed and you lost '..bagAmount..' bag(\'s) '..availableDrugs[drugType].label, 'error')
                stealingPed = ped
                stealData = {
                ...
                if pedDist < 1.5 then
                    MSCore.Functions.DrawText3D(pedCoords.x, pedCoords.y, pedCoords.z, '~g~E~w~ '..bagAmount..'x '..currentOfferDrug.label..' for $'..randomPrice..'? / ~g~G~w~ Decline offer')
                    if IsControlJustPressed(0, Keys["E"]) then
                        MSCore.Functions.Notify('Oferta aceite!', 'success')
                        TriggerServerEvent('ms-drugs:server:sellCornerDrugs', availableDrugs[drugType].item, bagAmount, randomPrice)
                        hasTarget = false
                    ...
                    if IsControlJustPressed(0, Keys["G"]) then
                        MSCore.Functions.Notify('Canceled offer!', 'error')
                        hasTarget = false
