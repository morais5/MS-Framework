...
AddEventHandler("RefuelCar",function()
    ...
    if curw == w then
        ...
        if DoesEntityExist(targetVehicle) then
            ...
            TriggerEvent('MSCore:Notify', "Refueled", "success")
        else
            TriggerEvent('MSCore:Notify', "No Target", "error")
        end
    else
        TriggerEvent('MSCore:Notify', "Need a Gas Can", "error")
    end
end)
...
AddEventHandler("RefuelCarServerReturn",function()
    ...
    refillVehicle()
    MSCore.Functions.Progressbar("refueling", "Refueling", timer, false, true, {
...
AddEventHandler("CarFuelAlarm",function()
    if not alarmset then
        alarmset = true
        local i = 5
        TriggerEvent('MSCore:Notify', "Low fuel.", "error")
        while i > 0 do
            ...
            if DoesEntityExist(veh) and IsEntityAVehicle(veh) and #(GetEntityCoords(veh) - GetEntityCoords(GLOBAL_PED)) < 5.0 then
                ...
                if IsControlJustPressed(1, Controlkey["generalUse"][1]) then
                    if curFuel >= 100 then
                        ...
                        TriggerEvent('MSCore:Notify', "Your gas tank is full", "error")
                    else
                        costs = math.ceil(costs)
                        MSCore.Functions.TriggerCallback('MSCore:HasMoney', function(hasMoney)
                            if hasMoney == true then
                                TriggerEvent('RefuelCarServerReturn')
                                TriggerEvent('MSCore:Notify', "Refueling your car", "success")
                            else
                                TriggerEvent('MSCore:Notify', "You dont have that amount", "error")
                            end
                        end, costs)
                    end
                end
