...
MSCore.Functions.CreateUseableItem("sniper_ammo", function(source, item)
            ...
            if WeaponSlot.info.quality ~= nil then
                for i = 1, RepeatAmount, 1 do
                    if WeaponSlot.info.quality - DecreaseAmount > 0 then
                        ...
                        WeaponSlot.info.quality = 0
                        TriggerClientEvent('inventory:client:UseWeapon', src, data)
                        TriggerClientEvent('MSCore:Notify', src, "Your gun is broken, you need to repair it before using it again.", "error")
                        break
                    end
                end
            else
...
MSCore.Commands.Add("repairweapon", "Weapon repair command for staff", {{name="hp", help="HP of your weapon"}}, true, function(source, args)
    ...
    if Player.PlayerData.items[data.slot] ~= nil then
        if Player.PlayerData.items[data.slot].info.quality ~= nil then
            if Player.PlayerData.items[data.slot].info.quality ~= 100 then
                if Player.Functions.RemoveMoney('cash', Config.WeaponRepairCotsts[WeaponClass]) then
                    ...
                    SetTimeout(Timeout, function()
                        ...
                        TriggerEvent('ms-phone:server:sendNewMailToOffline', Player.PlayerData.citizenid, {
                            sender = "Tyrone",
                            subject = "Repair",
                            message = "Your "..WeaponData.label.." is repaired u can pick it up on the spot. <br> <br> Peace madafaka"
                        })
                ...
                TriggerClientEvent("MSCore:Notify", src, "This weapon is not damaged..", "error")
                cb(false)
            end
        else
            TriggerClientEvent("MSCore:Notify", src, "This weapon is not damaged..", "error")
        ...
        TriggerClientEvent('MSCore:Notify', src, "You didn't have a gun in your hands..", "error")
    ...
    if Inventory[CurrentWeaponData.slot] ~= nil then
        if Inventory[CurrentWeaponData.slot].info.attachments ~= nil and next(Inventory[CurrentWeaponData.slot].info.attachments) ~= nil then
            ...
            if not HasAttach then
                ...
                TriggerClientEvent("MSCore:Notify", src, "You already have "..AttachmentData.label:lower().." on your gun.", "error", 3500)
            end
        else
    ...
    if Inventory[ItemData.slot] ~= nil then
        if Inventory[ItemData.slot].info.attachments ~= nil and next(Inventory[ItemData.slot].info.attachments) ~= nil then
            ...
            if HasAttach then
                ...
                TriggerClientEvent("MSCore:Notify", src, "You removed "..AttachmentComponent.label.." of your gun!", "error")
