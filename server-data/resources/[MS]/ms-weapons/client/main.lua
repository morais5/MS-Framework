            ...
            if CurrentWeaponData ~= nil and next(CurrentWeaponData) ~= nil then
                if IsPedShooting(GetPlayerPed(-1)) or IsControlJustPressed(0, 24) then
                    if CanShoot then
                        ...
                        MSCore.Functions.Notify("This weapon is damaged and cannot be used..", "error")
                        MultiplierAmount = 0
                    end
                end
            end
            ...
            if (total + retval) <= (retval + 1) then
                MSCore.Functions.Progressbar("taking_bullets", "Loading bullets..", math.random(4000, 6000), false, true, {
                    ...
                    if MSCore.Shared.Weapons[weapon] ~= nil then
                        ...
                        TriggerEvent('MSCore:Notify', retval.." Bullets loaded!", "success")
                    end
                end, function()
                    MSCore.Functions.Notify("Cancel..", "error")
                end)
            else
                MSCore.Functions.Notify("Your gun is already loaded..", "error")
            end
        else
            MSCore.Functions.Notify("You don't have a gun..", "error")
        end
    else
        MSCore.Functions.Notify("You don't have a gun..", "error")
    end
    ...
    if weapon ~= GetHashKey("WEAPON_UNARMED") then
        ...
        if Config.WeaponAttachments[WeaponData.name] ~= nil then
            if Config.WeaponAttachments[WeaponData.name][attachment] ~= nil then
                ...
                MSCore.Functions.Notify("This weapon does not support this accessory..", "error")
            end
        end
    else
        MSCore.Functions.Notify("You don't have a gun in your hand..", "error")
    end
end)
