    ...
    window.addEventListener('message', function (event) {
        ...
        if (data.action == "ui") {
            if (data.toggle) {
                ...
                setTimeout(function(){
                    $.post('http://ms-multicharacter/setupCharacters');
                    setTimeout(function(){
                        clearInterval(DotsInterval);
                        loadingProgress = 0;
                        originalText = "Recovering data";
                        $(".welcomescreen").fadeOut(150);
                        msMultiCharacters.fadeInDown('.character-info', '20%', 400);
                        msMultiCharacters.fadeInDown('.characters-list', '20%', 400);
                        $.post('http://ms-multicharacter/removeBlur');
                    }, 2000);
                }, 2000);
            } else {
...
$('.continue-btn').click(function(e){
    ...
    // msMultiCharacters.fadeOutUp('.welcomescreen', undefined, 400);
    // msMultiCharacters.fadeOutDown('.server-log', undefined, 400);
    // setTimeout(function(){
    //     msMultiCharacters.fadeInDown('.characters-list', '20%', 400);
    //     msMultiCharacters.fadeInDown('.character-info', '20%', 400);
    //     $.post('http://ms-multicharacter/setupCharacters');
...
$('.disconnect-btn').click(function(e){
    ...
    $.post('http://ms-multicharacter/closeUI');
    $.post('http://ms-multicharacter/disconnectButton');
});
...
$(document).on('click', '.character', function(e) {
    ...
    if (selectedChar === null) {
        ...
        if ((selectedChar).data('cid') == "") {
            ...
            $.post('http://ms-multicharacter/cDataPed', JSON.stringify({
                cData: cDataPed
            }));
        } else {
            $(selectedChar).addClass("char-selected");
...
$(document).on('click', '#create', function(e){
    e.preventDefault();
    $.post('http://ms-multicharacter/createNewCharacter', JSON.stringify({
    ...
    msMultiCharacters.fadeOutDown('.character-register', '125%', 400);
    refreshCharacters()
});

$(document).on('click', '#accept-delete', function(e){
    $.post('http://ms-multicharacter/removeCharacter', JSON.stringify({
...
function refreshCharacters() {
    $('.characters-list').html('<div class="character" id="char-1" data-cid=""><span id="slot-name">Empty Slot<span id="cid"></span></span></div><div class="character-btn" id="play"><p id="play-text">Select a character</p>')
...
$(document).on('click', '#play', function(e) {
    ...
    if (selectedChar !== null) {
        if (charData !== "") {
            $.post('http://ms-multicharacter/selectCharacter', JSON.stringify({
                cData: $(selectedChar).data('cData')
            }));
            // msMultiCharacters.fadeInDown('.welcomescreen', WelcomePercentage, 400);
            // msMultiCharacters.fadeInDown('.server-log', '25%', 400);
            setTimeout(function(){
                msMultiCharacters.fadeOutDown('.characters-list', "-40%", 400);
                msMultiCharacters.fadeOutDown('.character-info', "-40%", 400);
                msMultiCharacters.resetAll();
            }, 1500);
        } else {
            $('.characters-list').css("filter", "blur(2px)")
            $('.character-info').css("filter", "blur(2px)")
            msMultiCharacters.fadeInDown('.character-register', '25%', 400);
        }
    }
});
...
msMultiCharacters.fadeOutUp = function(element, time) {
    $(element).css({"display":"block"}).animate({top: "-80.5%",}, time, function(){
        $(element).css({"display":"none"});
    });
}

msMultiCharacters.fadeOutDown = function(element, percent, time) {
...
msMultiCharacters.fadeInDown = function(element, percent, time) {
    $(element).css({"display":"block"}).animate({top: percent,}, time);
}

msMultiCharacters.resetAll = function() {
