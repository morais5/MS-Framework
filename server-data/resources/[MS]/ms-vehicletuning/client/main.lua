...
AddEventHandler('MSCore:Client:OnPlayerLoaded', function()
    ...
    isLoggedIn = true
    MSCore.Functions.TriggerCallback('ms-vehicletuning:server:GetAttachedVehicle', function(plates)
        for k, v in pairs(plates) do
            Config.Plates[k].AttachedVehicle = v.AttachedVehicle
        end
    end)

    MSCore.Functions.TriggerCallback('ms-vehicletuning:server:GetDrivingDistances', function(retval)
...
Citizen.CreateThread(function()
    ...
    AddTextComponentSubstringPlayerName("Mechanics Office")
            ...
            if PlayerJob.name == "mechanic" then
                ...
                if onDuty then
                    if VehicleDistance < 20 then
                        ...
                        if VehicleDistance < 1 then
                            ...
                            if InVehicle then
                                DrawText3Ds(Config.Locations["vehicle"].x, Config.Locations["vehicle"].y, Config.Locations["vehicle"].z, '[E] Save vehicle ')
                                if IsControlJustPressed(0, Keys["E"]) then
                                    MSCore.Functions.DeleteVehicle(GetVehiclePedIsIn(GetPlayerPed(-1)))
                                end
                            else
                                DrawText3Ds(Config.Locations["vehicle"].x, Config.Locations["vehicle"].y, Config.Locations["vehicle"].z, '[E] Request vehicle')
                                if IsControlJustPressed(0, Keys["E"]) then
                                    if IsControlJustPressed(0, Keys["E"]) then
                                        VehicleList()
                ...
                if OnDutyDistance < 20 then
                    ...
                    if OnDutyDistance < 1 then
                        if onDuty then
                            DrawText3Ds(Config.Locations["duty"].x, Config.Locations["duty"].y, Config.Locations["duty"].z, "[E] Exit service")
                        else
                            DrawText3Ds(Config.Locations["duty"].x, Config.Locations["duty"].y, Config.Locations["duty"].z, "[E] Go on service")
                        end
                        if IsControlJustReleased(0, Keys["E"]) then
                            TriggerServerEvent("MSCore:ToggleDuty")
                        end
                    ...
                    for k, v in pairs(Config.Plates) do
                        if v.AttachedVehicle == nil then
                                ...
                                if PlateDistance < 2 then
                                    ...
                                    if IsPedInAnyVehicle(GetPlayerPed(-1)) then
                                        if not IsThisModelABicycle(GetEntityModel(veh)) then
                                            DrawText3Ds(v.coords.x, v.coords.y, v.coords.z + 0.3, "[E] Placing vehicle on the platform")
                                            if IsControlJustPressed(0, Config.Keys["E"]) then
                                                ...
                                                Wait(500)
                                                DoScreenFadeIn(250)
                                                TriggerServerEvent('ms-vehicletuning:server:SetAttachedVehicle', veh, k)
                                            end
                                        else
                                            MSCore.Functions.Notify("You can not put a bike on the platform!", "error")
                                        end
                                    end
                                end
                            end
                            ...
                            if PlateDistance < 3 then
                                inRange = true
                                DrawText3Ds(v.coords.x, v.coords.y, v.coords.z, "[E] Open interaction menu")
                                if IsControlJustPressed(0, Keys["E"]) then
                                    OpenMenu()
                                    Menu.hidden = not Menu.hidden
                                end
...
function VehicleOptions()
    ClearMenu()
    Menu.addButton("Remove Vehicle", "UnattachVehicle", nil)
...
function PartMenu(part)
    ClearMenu()
    Menu.addButton("Repair ("..MSCore.Shared.Items[Config.RepairCostAmount[part].item]["label"].." "..Config.RepairCostAmount[part].costs.."x)", "RepairPart", part)
    Menu.addButton("Back", "VehicleOptions", nil)
    Menu.addButton("Close", "CloseMenu", nil) 
end
...
function NoDamage(part)
    ClearMenu()
    Menu.addButton("This component is undamaged", "PartsMenu", part)
    Menu.addButton("Back", "VehicleOptions", nil)
    Menu.addButton("Close", "CloseMenu", nil) 
end
...
function RepairPart(part)
    ...
    MSCore.Functions.TriggerCallback('ms-inventory:server:GetStashItems', function(StashItems)
        for k, v in pairs(StashItems) do
            if v.name == PartData.item then
                if v.amount >= PartData.costs then
                    MSCore.Functions.Progressbar("repair_part", Config.ValuesLabels[part].." A ser reparado", math.random(5000, 10000), false, true, {
                        ...
                        TriggerEvent('ms-vehicletuning:client:RepaireeePart', part)
                        TriggerServerEvent('ms-inventory:server:SaveStashItems', "mechanicstash", StashItems)
                    ...
                    MSCore.Functions.Notify('You dont have enough materials in the stash..', 'error')
                end
                break
            else
                MSCore.Functions.Notify('You dont have enough materials in the stash..', 'error')
...
RegisterNetEvent('ms-vehicletuning:client:RepaireeePart')
AddEventHandler('ms-vehicletuning:client:RepaireeePart', function(part)
    ...
    MSCore.Functions.Notify("The "..Config.ValuesLabels[part].." was repaired!")
end)
...
function UnattachVehicle()
    ...
    TriggerServerEvent('ms-vehicletuning:server:SetAttachedVehicle', false, ClosestPlate)
end

RegisterNetEvent('ms-vehicletuning:client:SetAttachedVehicle')
AddEventHandler('ms-vehicletuning:client:SetAttachedVehicle', function(veh, key)
    if veh ~= false then
            ...
            if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, vehpos.x, vehpos.y, vehpos.z, true) < 5.0 then
                if not IsThisModelABicycle(GetEntityModel(veh)) then
                    ...
                    if VehicleStatus[plate] ~= nil then 
                        ...
                        MSCore.Functions.Notify("Unknown state..", "error")
                    end
                else
                    MSCore.Functions.Notify("Invalid vehicle..", "error")
                end
            else
                MSCore.Functions.Notify("Not close enough of the vehicle..", "error")
            end
        else
            MSCore.Functions.Notify("You have to enter the vehicle first..", "error")
        end
    else
        MSCore.Functions.Notify("You have to be out of the vehicle..", "error")
    end
end)
...
AddEventHandler('vehiclemod:client:fixEverything', function()
    if (IsPedInAnyVehicle(GetPlayerPed(-1), false)) then
        ...
        if not IsThisModelABicycle(GetEntityModel(veh)) and GetPedInVehicleSeat(veh, -1) == GetPlayerPed(-1) then
            ...
            MSCore.Functions.Notify("You are not in the driver's seat of the vehicle..", "error")
            end
        else
                ...
                if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, vehpos.x, vehpos.y, vehpos.z, true) < 5.0 then
                    if not IsThisModelABicycle(GetEntityModel(veh)) then
                        ...
                        if VehicleStatus[plate] ~= nil and VehicleStatus[plate][part] ~= nil then
                            ...
                            MSCore.Functions.Progressbar("repair_advanced", "Repairing the vehicle..", lockpickTime, false, true, {
                                disableMovement = true,
                                disableCarMovement = true,
                                disableMouse = false,
                                disableCombat = true,
                                ...
                                if part == "body" then
                                    ...
                                    TriggerServerEvent("MSCore:Server:RemoveItem", Config.RepairCost[part], needAmount)
                                    TriggerEvent("inventory:client:ItemBox", MSCore.Shared.Items[Config.RepairCost[part]], "remove")
                                elseif part ~= "engine" then
                                    TriggerServerEvent("vehiclemod:server:updatePart", plate, part, GetVehicleStatus(plate, part) + level)
                                    TriggerServerEvent("MSCore:Server:RemoveItem", Config.RepairCost[part], level)
                                    TriggerEvent("inventory:client:ItemBox", MSCore.Shared.Items[Config.RepairCost[part]], "remove")
                                end
                            end, function() -- Cancel
                                openingDoor = false
                            ...
                            MSCore.Functions.Notify("Invalid component.", "error")
                        end
                    else
                        MSCore.Functions.Notify("Invalid vehicle..", "error")
                    end
                else
                    MSCore.Functions.Notify("Not near the vehicle..", "error")
                end
            else
                MSCore.Functions.Notify("You have to enter the vehicle first..", "error")
            end
        else
            MSCore.Functions.Notify("Not in a vehicle..", "error")
        end
    -- end
end)
...
function CanReapair()
    ...
    for k, v in pairs(Config.Businesses) do
        retval = exports['ms-companies']:IsEmployee(v)
    end
    return retval
end
...
function SendStatusMessage(statusList)
    if statusList ~= nil then 
        TriggerEvent('chat:addMessage', {
            ...
            args = {'Vehicle Status', round(statusList["engine"]) .. "/" .. Config.MaxStatusValues["engine"] .. " ("..MSCore.Shared.Items["advancedrepairkit"]["label"]..")", round(statusList["body"]) .. "/" .. Config.MaxStatusValues["body"] .. " ("..MSCore.Shared.Items[Config.RepairCost["body"]]["label"]..")", round(statusList["radiator"]) .. "/" .. Config.MaxStatusValues["radiator"] .. ".0 ("..MSCore.Shared.Items[Config.RepairCost["radiator"]]["label"]..")", round(statusList["axle"]) .. "/" .. Config.MaxStatusValues["axle"] .. ".0 ("..MSCore.Shared.Items[Config.RepairCost["axle"]]["label"]..")", round(statusList["brakes"]) .. "/" .. Config.MaxStatusValues["brakes"] .. ".0 ("..MSCore.Shared.Items[Config.RepairCost["brakes"]]["label"]..")", round(statusList["clutch"]) .. "/" .. Config.MaxStatusValues["clutch"] .. ".0 ("..MSCore.Shared.Items[Config.RepairCost["clutch"]]["label"]..")", round(statusList["fuel"]) .. "/" .. Config.MaxStatusValues["fuel"] .. ".0 ("..MSCore.Shared.Items[Config.RepairCost["fuel"]]["label"]..")"}
        })
    end
end
