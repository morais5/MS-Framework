...
Citizen.CreateThread(function()
    for k, v in pairs(msCustoms.Locations) do
...
function OpenCustoms(shop, k)
    ...
    CurrentVehicleData = MSCore.Functions.GetVehicleProperties(veh)
    SetEntityCoords(veh, shop["coords"].x, shop["coords"].y, shop["coords"].z, 0.0, 0.0, 0.0, false)
    SetEntityHeading(veh, shop["coords"].h)
    FreezeEntityPosition(veh, true)

    if k == 7 then
        MSCore.Functions.TriggerCallback('ms-vehicletuning:server:IsMechanicAvailable', function(Mechanic)
            if Mechanic < 2 then
            ...
            TriggerServerEvent('ms-customs:server:UpdateBusyState', k, true)
            CurrentShop = k
        end)
    else
        SetNuiFocus(true, false)
...
RegisterNUICallback('CanRepairVehicle', function(data, cb)
    ...
    MSCore.Functions.TriggerCallback('ms-customs:server:CanPurchase', function(CanBuy)
        if CanBuy then
	        SetVehicleFixed(veh)
            TriggerServerEvent("InteractSound_SV:PlayOnSource", "airwrench", 0.1)
            cb(true)
        else
            MSCore.Functions.Notify('You dont have enough money..', 'error')
            cb(false)
        end
    end, data.price)
end)
...
function IsVehicleBlacklisted(veh)
    local retval = false
    for _, vehicle in pairs(msCustoms.BlacklistedVehicles) do
...
function GetAvailableMods()
    ...
    if MSCore.Shared.VehicleModels[GetEntityModel(veh)] ~= nil then
        model = MSCore.Shared.VehicleModels[GetEntityModel(veh)].model
        VehicleData = MSCore.Shared.Vehicles[model]
    ...
    local mods = {}
    for k, v in pairs(msCustoms.Mods) do
                            ...
                            table.insert(mods, {
                                ...
                                buttons = {
                                    ...
                                    },
                                    {
                                        id = "wheelcolors",
                                        name = "Wheel Colors",
                                        buttons = msCustoms.WheelColors,
                                    },
                                    {
                                        id = "wheelaccessories",
                                        name = "Wheel Accessoires",
                                        buttons = msCustoms.WheelAccessories,
                                    },
                                },
                            })
                        else
...
Citizen.CreateThread(function()
    while true do
        ...
        for k, loc in pairs(msCustoms.Locations) do
...
RegisterNUICallback('CloseMenu', function()
    ...
    TriggerServerEvent('ms-customs:server:UpdateBusyState', CurrentShop, false)
    ShoppingCart.buttons = {}
    MSCore.Functions.SetVehicleProperties(veh, CurrentVehicleData)
    CurrentShop = nil
end)

RegisterNUICallback('print', function(data, cb)
    -- TriggerServerEvent('ms-customs:print', data.print)
end)
...
RegisterNetEvent('ms-customs:client:UpdateBusyState')
AddEventHandler('ms-customs:client:UpdateBusyState', function(k, bool)
    ...
    if data.data ~= nil and next(data.data) ~= nil then
        if data.data.id ~= nil then
            if data.data.id ~= "respray" and data.data.id ~= "wheels" and data.data.id ~= "wheelcolors" and data.data.id ~= "wheelaccessories" then
                if data.data.id >= 0 and data.data.id <= 46 then
                    if data.data.id == 18 or data.data.id == 20 or data.data.id == 22 then
                        if not CurrentVehicleData[msCustoms.indexToName[data.data.id]] then
                            currentmod = -1
                        else
                            currentmod = 0
                        end
                    else
                        currentmod = CurrentVehicleData[msCustoms.indexToName[data.data.id]]
                    end
                end
            ...
            elseif data.data.id == "wheelcolors" then
                local pearlescentColor, wheelColor = GetVehicleExtraColours(veh)
                for k, v in pairs(msCustoms.WheelColors) do
    ...
    MSCore.Functions.TriggerCallback('ms-customs:server:CanPurchase', function(CanBuy)
        if CanBuy then
            ...
            if ShoppingCart.buttons ~= nil and next(ShoppingCart.buttons) ~= nil then
                ...
                TriggerServerEvent("ms-log:server:sendLog", PlayerData.citizenid, "buy", {upgrades = String, citizenid = PlayerData.citizenid})
	            TriggerServerEvent("ms-log:server:CreateLog", "bennys", "buy", "green", "**" .. PlayerData.name .. "** (citizenid: *" .. PlayerData.citizenid .. "* | id: *(" .. PlayerData.source .. ")* has paid **$" .. TotalPrice .. "** for the following upgrade\'s: \n" .. String)
            end
            ...
            TriggerServerEvent('ms-customs:server:SaveVehicleProps', CurrentVehicleData)
        else
            MSCore.Functions.Notify('You dont have enough money..', 'error')
        end
    end, TotalPrice)
...
function GetTyreSmokeKey()
    ...
    for k, v in pairs(msCustoms.WheelAccessories) do
    ...
    if data.data ~= nil then
        if ShoppingCart.buttons ~= nil and next(ShoppingCart.buttons) ~= nil then
            for k, v in pairs(ShoppingCart.buttons) do
                ...
                elseif v.modtype == "wheelcolor" then
                    for k, mod in pairs(data.data) do
                        for _, color in pairs(msCustoms.WheelColors) do
