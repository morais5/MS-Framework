...
RentVehicle = function(selectedVehicle)
    ...
    if IsPedInAnyVehicle(ped) then
        MSCore.Functions.Notify('this can only when not in a vehicle..', 'error')
        return
    end

    local vehiclePlate = "msR"..math.random(1, 100)
    TriggerServerEvent('ms-vehiclerental:server:SetVehicleRented', vehiclePlate, true, Config.RentalVehicles[selectedVehicle])

    RentedVehicleData = selectedVehicle
end

RegisterNetEvent('ms-vehiclerental:server:SpawnRentedVehicle')
AddEventHandler('ms-vehiclerental:server:SpawnRentedVehicle', function(vehiclePlate, vehicleData)
    local ped = GetPlayerPed(-1)
    local coords = {
...
ReturnVehicle = function()
    ...
    MSCore.Functions.Notify('You do not have a deposit open..', 'error')
end
...
AcceptReturn = function()
    ...
    if noSpace(VehiclePlate) ~= noSpace(RentedVehiclePlate) then
        MSCore.Functions.Notify('This is not a rental vehicle..', 'error')
    else
        TriggerServerEvent('ms-vehiclerental:server:SetVehicleRented', RentedVehiclePlate, false, Config.RentalVehicles[RentedVehicleData])
        MSCore.Functions.DeleteVehicle(CurrentVehicle)
