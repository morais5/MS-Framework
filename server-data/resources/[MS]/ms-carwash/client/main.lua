                        ...
                        if Driver == PlayerPed then
                            if not washingVehicle then
                                ...
                                if IsControlJustPressed(0, Keys["E"]) then
                                    TriggerServerEvent('ms-carwash:server:washCar')
                                end
                            else
...
RegisterNetEvent('ms-carwash:client:washCar')
AddEventHandler('ms-carwash:client:washCar', function()
    local PlayerPed = GetPlayerPed(-1)
    local PedVehicle = GetVehiclePedIsIn(PlayerPed)
    local Driver = GetPedInVehicleSeat(PedVehicle, -1)
    ...
    MSCore.Functions.Progressbar("search_cabin", "Wash the vehicle..", math.random(4000, 8000), false, true, {
