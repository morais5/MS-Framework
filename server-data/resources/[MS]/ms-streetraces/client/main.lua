                    ...
                    if Races[k] ~= nil then
                        if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, Races[k].startx, Races[k].starty, Races[k].startz, true) < 15.0 and not Races[k].started then
                            ...
                            if IsControlJustReleased(0, Keys["H"]) then
                                TriggerServerEvent("ms-streetraces:JoinRace", k)
                            end
                        end
                    end
            ...
            if RaceId ~= 0 and InRace then
                if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, Races[RaceId].endx, Races[RaceId].endy, pos.z, true) < 250.0 and Races[RaceId].started then
                    ...
                    if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, Races[RaceId].endx, Races[RaceId].endy, pos.z, true) < 15.0 then
                        TriggerServerEvent("ms-streetraces:RaceWon", RaceId)
                        InRace = false
                    end
                end
            end
...
RegisterNetEvent('ms-streetraces:StartRace')
AddEventHandler('ms-streetraces:StartRace', function(race)
...
RegisterNetEvent('ms-streetraces:RaceDone')
AddEventHandler('ms-streetraces:RaceDone', function(race, winner)
    if RaceId ~= 0 and RaceId == race then
        RaceId = 0
        InRace = false
        MSCore.Functions.Notify(" The race endded. "..winner.. "!")
    end
end)

RegisterNetEvent('ms-streetraces:StopRace')
AddEventHandler('ms-streetraces:StopRace', function()
    RaceId = 0
    InRace = false
end)

RegisterNetEvent('ms-streetraces:CreateRace')
AddEventHandler('ms-streetraces:CreateRace', function(amount)
    local pos = GetEntityCoords(GetPlayerPed(-1), true)
    ...
    if DoesBlipExist(WaypointHandle) then
        ...
        if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, cx, cy, groundZ, true) > 500.0 then
            ...
            TriggerServerEvent("ms-streetraces:NewRace", race)
            MSCore.Functions.Notify("Race made for $"..amount..",-!", "success")
        else
            MSCore.Functions.Notify("end position to close!", "error")
        end
    else
        MSCore.Functions.Notify("You have to place a marker!", "error")
    end
end)

RegisterNetEvent('ms-streetraces:SetRace')
AddEventHandler('ms-streetraces:SetRace', function(RaceTable)
    Races = RaceTable
end)

RegisterNetEvent('ms-streetraces:SetRaceId')
AddEventHandler('ms-streetraces:SetRaceId', function(race)
    RaceId = race
    SetNewWaypoint(Races[RaceId].endx, Races[RaceId].endy)
end)
...
function RaceCountDown()
    ...
    while RaceCount ~= 0 do
        ...
        MSCore.Functions.Notify(RaceCount, 'primary', 800)
        Citizen.Wait(1000)
    ...
    MSCore.Functions.Notify("GOOOOOOOOO!!!")
end
