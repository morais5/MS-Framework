...
$(document).on('click', '.twitter-new-tweet', function(e){
    ...
    ms.Phone.Animations.TopSlideDown(".twitter-new-tweet-tab", 450, 0);
});

ms.Phone.Notifications.LoadTweets = function(Tweets) {
    Tweets = Tweets.reverse();
    if (Tweets !== null && Tweets !== undefined && Tweets !== "" && Tweets.length > 0) {
        $(".twitter-home-tab").html("");
        $.each(Tweets, function(i, Tweet){
            var TwtMessage = ms.Phone.Functions.FormatTwitterMessage(Tweet.message);
            var today = new Date();
...
ms.Phone.Notifications.LoadMentionedTweets = function(Tweets) {
    Tweets = Tweets.reverse();
    if (Tweets.length > 0) {
...
ms.Phone.Functions.FormatTwitterMessage = function(TwitterMessage) {
...
$(document).on('click', '#send-tweet', function(e){
    ...
    if (TweetMessage != "") {
        ...
        ms.Phone.Notifications.Add("fab fa-twitter", "Twitter", "Write a Tweet!", "#1DA1F2");
    }
});
...
function CopyMentionTag(elem) {
    ...
    ms.Phone.Notifications.Add("fab fa-twitter", "Twitter", $(elem).data('mentiontag')+ " copied!", "rgb(27, 149, 224)", 1250);
    document.execCommand("copy");
    $temp.remove();
}

ms.Phone.Notifications.LoadHashtags = function(hashtags) {
    if (hashtags !== null) {
...
ms.Phone.Notifications.LoadHashtagMessages = function(Tweets) {
    Tweets = Tweets.reverse();
    if (Tweets !== null && Tweets !== undefined && Tweets !== "" && Tweets.length > 0) {
...
$(document).on('click', '.twitter-hashtag', function(event){
    ...
    ms.Phone.Notifications.LoadHashtagMessages(TweetData.messages);
