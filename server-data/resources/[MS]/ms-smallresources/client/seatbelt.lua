    ...
    while true do
        ...
        if IsControlJustReleased(0, Keys["G"]) then 
            if not harnessOn then
                ...
                MSCore.Functions.Progressbar("harness_equip", "Unscrewing the 3-point belt..", 5000, false, true, {
...
AddEventHandler('seatbelt:client:UseHarness', function(ItemData)
    ...
    if inveh and not IsThisModelABike(GetEntityModel(GetVehiclePedIsIn(ped))) then
        if not harnessOn then
            MSCore.Functions.Progressbar("harness_equip", "Tightening the 3-point Belt..", 5000, false, true, {
                disableMovement = false,
                disableCarMovement = false,
                disableMouse = false,
                disableCombat = true,
        ...
        MSCore.Functions.Notify('You are not in a car.', 'error', 3500)
    end
end)
