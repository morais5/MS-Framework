...
AddEventHandler("consumables:client:UseJoint", function()
    MSCore.Functions.Progressbar("smoke_joint", "A dar umas passas..", 1500, false, true, {
        disableMovement = false,
        disableCarMovement = false,
		disableMouse = false,
		disableCombat = true,
    }, {}, {}, {}, function() -- Done
        TriggerEvent("inventory:client:ItemBox", MSCore.Shared.Items["joint"], "remove")
...
AddEventHandler("consumables:client:UseParachute", function()
    EquipParachuteAnim()
    MSCore.Functions.Progressbar("use_parachute", "parachute using..", 5000, false, true, {
        disableMovement = false,
        disableCarMovement = false,
		disableMouse = false,
		disableCombat = true,
        ...
        TriggerEvent("inventory:client:ItemBox", MSCore.Shared.Items["parachute"], "remove")
        ...
        TriggerEvent('ms-clothing:client:loadOutfit', ParachuteData)
...
AddEventHandler("consumables:client:ResetParachute", function()
    if ParachuteEquiped then 
        EquipParachuteAnim()
        MSCore.Functions.Progressbar("reset_parachute", "Opening the parachute..", 40000, false, true, {
            disableMovement = false,
            disableCarMovement = false,
            disableMouse = false,
            disableCombat = true,
            ...
            local ped = GetPlayerPed(-1)
            TriggerEvent("inventory:client:ItemBox", MSCore.Shared.Items["parachute"], "add")
            local ParachuteRemoveData = { 
                outfitData = { 
                    ["bag"] = { item = 0, texture = 0} -- Nek / Das
                }
            }
            TriggerEvent('ms-clothing:client:loadOutfit', ParachuteRemoveData)
            TaskPlayAnim(ped, "clothingshirt", "exit", 8.0, 1.0, -1, 49, 0, 0, 0, 0)
            TriggerServerEvent("ms-smallpenis:server:AddParachute")
            ParachuteEquiped = false
        end)
    else
        MSCore.Functions.Notify("You don't have an open parachute!", "error")
    end
end)
...
--         TriggerEvent("inventory:client:Itembox", MSCore.Shared.Items["smoketrailred"], "remove")
--     else
--         MSCore.Functions.Notify("You need to have a paracute to activate smoke!", "error")    
--     end
-- end)

RegisterNetEvent("consumables:client:UseArmor")
AddEventHandler("consumables:client:UseArmor", function()
    MSCore.Functions.Progressbar("use_armor", "Wearing the Vest..", 5000, false, true, {
        disableMovement = false,
        disableCarMovement = false,
		disableMouse = false,
		disableCombat = true,
    }, {}, {}, {}, function() -- Done
        TriggerEvent("inventory:client:ItemBox", MSCore.Shared.Items["armor"], "remove")
        TriggerServerEvent('hospital:server:SetArmor', 75)
        TriggerServerEvent("MSCore:Server:RemoveItem", "armor", 1)
...
AddEventHandler("consumables:client:UseHeavyArmor", function()
    ...
    MSCore.Functions.Progressbar("use_heavyarmor", "Wearing the Vest..", 5000, false, true, {
        disableMovement = false,
        disableCarMovement = false,
		disableMouse = false,
		disableCombat = true,
        ...
        TriggerEvent("inventory:client:ItemBox", MSCore.Shared.Items["heavyarmor"], "remove")
        TriggerServerEvent("MSCore:Server:RemoveItem", "heavyarmor", 1)
...
AddEventHandler("consumables:client:ResetArmor", function()
    ...
    if currentVest ~= nil and currentVestTexture ~= nil then 
        MSCore.Functions.Progressbar("remove_armor", "Removing the Vest..", 2500, false, true, {
            disableMovement = false,
            disableCarMovement = false,
            disableMouse = false,
            disableCombat = true,
            ...
            SetPedArmour(ped, 0)
            TriggerEvent("inventory:client:ItemBox", MSCore.Shared.Items["heavyarmor"], "add")
            TriggerServerEvent("MSCore:Server:AddItem", "heavyarmor", 1)
        end)
    else
        MSCore.Functions.Notify("You are not wearing a vest...", "error")
    end
end)
...
AddEventHandler("consumables:client:DrinkAlcohol", function(itemName)
    ...
    MSCore.Functions.Progressbar("snort_coke", "Consuming..", math.random(3000, 6000), false, true, {
        disableMovement = false,
        disableCarMovement = false,
        disableMouse = false,
        disableCombat = true,
        ...
        TriggerEvent("inventory:client:ItemBox", MSCore.Shared.Items[itemName], "remove")
        TriggerServerEvent("MSCore:Server:RemoveItem", itemName, 1)
        TriggerServerEvent("MSCore:Server:SetMetaData", "thirst", MSCore.Functions.GetPlayerData().metadata["thirst"] + Consumeables[itemName])
        alcoholCount = alcoholCount + 1
...
AddEventHandler("consumables:client:Cokebaggy", function()
    MSCore.Functions.Progressbar("snort_coke", "Consuming..", math.random(5000, 8000), false, true, {
        disableMovement = false,
        disableCarMovement = false,
        disableMouse = false,
        disableCombat = true,
        ...
        TriggerServerEvent("MSCore:Server:RemoveItem", "cokebaggy", 1)
        TriggerEvent("inventory:client:ItemBox", MSCore.Shared.Items["cokebaggy"], "remove")
...
AddEventHandler("consumables:client:Crackbaggy", function()
    MSCore.Functions.Progressbar("snort_coke", "Smoking crack..", math.random(7000, 10000), false, true, {
        disableMovement = false,
        disableCarMovement = false,
        disableMouse = false,
        disableCombat = true,
        ...
        StopAnimTask(GetPlayerPed(-1), "switch@trevor@trev_smoking_meth", "trev_smoking_meth_loop", 1.0)
        TriggerServerEvent("MSCore:Server:RemoveItem", "crack_baggy", 1)
        TriggerEvent("inventory:client:ItemBox", MSCore.Shared.Items["crack_baggy"], "remove")
...
AddEventHandler('consumables:client:EcstasyBaggy', function()
    MSCore.Functions.Progressbar("use_ecstasy", "Popping Pills", 3000, false, true, {
        disableMovement = false,
        disableCarMovement = false,
		disableMouse = false,
		disableCombat = true,
        ...
        TriggerServerEvent("MSCore:Server:RemoveItem", "xtcbaggy", 1)
        TriggerEvent("inventory:client:ItemBox", MSCore.Shared.Items["xtcbaggy"], "remove")
        EcstasyEffect()
    end, function() -- Cancel
        StopAnimTask(GetPlayerPed(-1), "mp_suicide", "pill", 1.0)
        MSCore.Functions.Notify("Cancel", "error")
...
AddEventHandler("consumables:client:Eat", function(itemName)
    ...
    MSCore.Functions.Progressbar("eat_something", "Eating..", 2500, false, true, {
        disableMovement = false,
        disableCarMovement = false,
		disableMouse = false,
		disableCombat = true,
        ...
        TriggerEvent("inventory:client:ItemBox", MSCore.Shared.Items[itemName], "remove")
        TriggerEvent('animations:client:EmoteCommandStart', {"c"})
        TriggerServerEvent("MSCore:Server:SetMetaData", "hunger", MSCore.Functions.GetPlayerData().metadata["hunger"] + Consumeables[itemName])
        TriggerServerEvent('ms-hud:Server:RelieveStress', math.random(2, 4))
    end)
end)
...
AddEventHandler("consumables:client:Drink", function(itemName)
    TriggerEvent('animations:client:EmoteCommandStart', {"drink"})
    MSCore.Functions.Progressbar("drink_something", "Drinking..", 2500, false, true, {
        disableMovement = false,
        disableCarMovement = false,
		disableMouse = false,
		disableCombat = true,
...
function JointEffect()
    ...
    --                 TriggerServerEvent('ms-hud:Server:RelieveStress', math.random(14, 18))
    --             end
    --             if weedTime <= 0 then
    --                 onWeed = false
    --             end
