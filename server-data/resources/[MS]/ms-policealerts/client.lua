...
RegisterNetEvent('ms-policealerts:ToggleDuty')
AddEventHandler('ms-policealerts:ToggleDuty', function(Duty)
    PlayerJob.onduty = Duty
end)

RegisterNetEvent('ms-policealerts:client:AddPoliceAlert')
AddEventHandler('ms-policealerts:client:AddPoliceAlert', function(data, forBoth)
    if forBoth then
...
RegisterNUICallback('SetWaypoint', function(data)
    ...
    MSCore.Functions.Notify('GPS set!')
    SetNuiFocus(false, false)
    SetNuiFocusKeepInput(false, false)
    MouseActive = false
end)
