...
AddEventHandler('pulisci:moneblak', function()
    ...
    if Item == nil then
        TriggerClientEvent('MSCore:Notify', source, 'Não tens dinheiro sujo para lavar, burro do crh :/!', "error", 8000)  
    else
        if Item.amount >= Item.amount then
		   ...
		   xPlayer.Functions.AddMoney("cash", Item.amount, "sold-pawn-items")
            ...
            TriggerClientEvent('MSCore:Notify', _source, 'Não tens dinheiro sujo para lavar, burro do crh :/!', "error", 10000)  
