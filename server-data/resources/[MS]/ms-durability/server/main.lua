...
MSCore.Functions.CreateCallback("weapons:server:GetConfig", function(source, cb)
...
--     MSCore.Functions.ExecuteSql(false, "SELECT * FROM `playerammo` WHERE `citizenid` = '"..Player.PlayerData.citizenid.."'", function(result)
...
MSCore.Functions.CreateCallback("weapon:server:GetWeaponAmmo", function(source, cb, WeaponData)
...
MSCore.Functions.CreateUseableItem("pistol_ammo", function(source, item)
    TriggerClientEvent("weapon:client:AddAmmo", source, "AMMO_PISTOL", 12, item)
end)

MSCore.Functions.CreateUseableItem("rifle_ammo", function(source, item)
    TriggerClientEvent("weapon:client:AddAmmo", source, "AMMO_RIFLE", 30, item)
end)

MSCore.Functions.CreateUseableItem("smg_ammo", function(source, item)
    TriggerClientEvent("weapon:client:AddAmmo", source, "AMMO_SMG", 20, item)
end)

MSCore.Functions.CreateUseableItem("shotgun_ammo", function(source, item)
    TriggerClientEvent("weapon:client:AddAmmo", source, "AMMO_SHOTGUN", 10, item)
end)

MSCore.Functions.CreateUseableItem("mg_ammo", function(source, item)
...
AddEventHandler('weapons:server:UpdateWeaponQuality', function(data, RepeatAmount)
    local src = source
    local Player = MSCore.Functions.GetPlayer(src)
    local WeaponData = MSCore.Shared.Weapons[GetHashKey(data.name)]
            ...
            if WeaponSlot.info.quality ~= nil then
                for i = 1, RepeatAmount, 1 do
                    if WeaponSlot.info.quality - DecreaseAmount > 0 then
                        ...
                        TriggerClientEvent('MSCore:Notify', src, "Your weapon is broken, u need to repair it before u can use it again.", "error")
                        break
                    end
                end
            else
    ...
    if Config.UseMultiplier[data.name] ~= nil then
        if ItemSlot.info.quality ~= nil then
            if ItemSlot.info.quality - DecreaseAmount > 0 then
                ...
                TriggerClientEvent('MSCore:Notify', src, "Your item is broken and cant be used..", "error")
            end
        end
    end
    Player.Functions.SetInventory(Player.PlayerData.items)
...
MSCore.Commands.Add("repairweapon", "Repair weapon command for staf", {{name="hp", help="HP of your weapon"}}, true, function(source, args)
    TriggerClientEvent('weapons:client:SetWeaponQuality', source, tonumber(args[1]))
end, "god")

MSCore.Commands.Add("setdurability", "Set durability command for staf", {{name="slot", help="Slot of the item"}, {name="hp", help="HP of your weapon"}}, true, function(source, args)
...
MSCore.Functions.CreateCallback("weapons:server:RepairWeapon", function(source, cb, RepairPoint, data)
    local src = source
    local Player = MSCore.Functions.GetPlayer(src)
    ...
    if Player.PlayerData.items[data.slot] ~= nil then
        if Player.PlayerData.items[data.slot].info.quality ~= nil then
            if Player.PlayerData.items[data.slot].info.quality ~= 100 then
                if Player.Functions.RemoveMoney('cash', Config.WeaponRepairCotsts[WeaponClass]) then
                    ...
                    TriggerClientEvent('inventory:client:ItemBox', src, MSCore.Shared.Items[data.name], "remove")
                    ...
                    SetTimeout(Timeout, function()
                        ...
                        TriggerClientEvent('weapons:client:SyncRepairShops', -1, Config.WeaponRepairPoints[RepairPoint], RepairPoint)
                        TriggerEvent('ms-phone:server:sendNewMailToOffline', Player.PlayerData.citizenid, {
                            sender = "Tyrone",
                            subject = "Reperatie",
                ...
                TriggerClientEvent("MSCore:Notify", src, "This weapon is not dammaged..", "error")
                cb(false)
            end
        else
            TriggerClientEvent("MSCore:Notify", src, "This weapon is not dammaged..", "error")
        ...
        TriggerClientEvent('MSCore:Notify', src, "You diddent have a weapon in ur hands..", "error")
...
AddEventHandler("weapons:server:TakeBackWeapon", function(k, data)
    ...
    TriggerClientEvent('inventory:client:ItemBox', src, MSCore.Shared.Items[itemdata.name], "add")
...
MSCore.Functions.CreateUseableItem("pistol_suppressor", function(source, item)
    local Player = MSCore.Functions.GetPlayer(source)
    TriggerClientEvent("weapons:client:EquipAttachment", source, item, "suppressor")
end)

MSCore.Functions.CreateUseableItem("pistol_extendedclip", function(source, item)
    local Player = MSCore.Functions.GetPlayer(source)
    TriggerClientEvent("weapons:client:EquipAttachment", source, item, "extendedclip")
end)

MSCore.Functions.CreateUseableItem("pistol_flashlight", function(source, item)
    local Player = MSCore.Functions.GetPlayer(source)
...
MSCore.Functions.CreateUseableItem("smg_suppressor", function(source, item)
    local Player = MSCore.Functions.GetPlayer(source)
    TriggerClientEvent("weapons:client:EquipAttachment", source, item, "suppressor")
end)

MSCore.Functions.CreateUseableItem("smg_extendedclip", function(source, item)
    local Player = MSCore.Functions.GetPlayer(source)
    TriggerClientEvent("weapons:client:EquipAttachment", source, item, "extendedclip")
end)

MSCore.Functions.CreateUseableItem("smg_drummag", function(source, item)
    local Player = MSCore.Functions.GetPlayer(source)
    TriggerClientEvent("weapons:client:EquipAttachment", source, item, "drummag")
end)

MSCore.Functions.CreateUseableItem("smg_flashlight", function(source, item)
    local Player = MSCore.Functions.GetPlayer(source)
    TriggerClientEvent("weapons:client:EquipAttachment", source, item, "flashlight")
end)

MSCore.Functions.CreateUseableItem("smg_scope", function(source, item)
    local Player = MSCore.Functions.GetPlayer(source)
...
MSCore.Functions.CreateUseableItem("shotgun_suppressor", function(source, item)
    local Player = MSCore.Functions.GetPlayer(source)
    TriggerClientEvent("weapons:client:EquipAttachment", source, item, "suppressor")
end)

MSCore.Functions.CreateUseableItem("shotgun_flashlight", function(source, item)
    local Player = MSCore.Functions.GetPlayer(source)
    TriggerClientEvent("weapons:client:EquipAttachment", source, item, "flashlight")
end)

MSCore.Functions.CreateUseableItem("shotgun_extendedclip", function(source, item)
    local Player = MSCore.Functions.GetPlayer(source)
    TriggerClientEvent("weapons:client:EquipAttachment", source, item, "extendedclip")
end)

MSCore.Functions.CreateUseableItem("shotgun_drummag", function(source, item)
    local Player = MSCore.Functions.GetPlayer(source)
    TriggerClientEvent("weapons:client:EquipAttachment", source, item, "drummag")
end)
...
MSCore.Functions.CreateUseableItem("rifle_extendedclip", function(source, item)
    local Player = MSCore.Functions.GetPlayer(source)
    TriggerClientEvent("weapons:client:EquipAttachment", source, item, "extendedclip")
end)

MSCore.Functions.CreateUseableItem("rifle_drummag", function(source, item)
    local Player = MSCore.Functions.GetPlayer(source)
    TriggerClientEvent("weapons:client:EquipAttachment", source, item, "drummag")
end)

MSCore.Functions.CreateUseableItem("rifle_scope", function(source, item)
    local Player = MSCore.Functions.GetPlayer(source)
    TriggerClientEvent("weapons:client:EquipAttachment", source, item, "scope")
end)

MSCore.Functions.CreateUseableItem("rifle_flashlight", function(source, item)
    local Player = MSCore.Functions.GetPlayer(source)
    TriggerClientEvent("weapons:client:EquipAttachment", source, item, "flashlight")
end)

MSCore.Functions.CreateUseableItem("rifle_suppressor", function(source, item)
    local Player = MSCore.Functions.GetPlayer(source)
    TriggerClientEvent("weapons:client:EquipAttachment", source, item, "suppressor")
end)

MSCore.Functions.CreateUseableItem("rifle_grip", function(source, item)
    local Player = MSCore.Functions.GetPlayer(source)
...
MSCore.Functions.CreateUseableItem("sniper_flashlight", function(source, item)
    local Player = MSCore.Functions.GetPlayer(source)
    TriggerClientEvent("weapons:client:EquipAttachment", source, item, "flashlight")
end)

MSCore.Functions.CreateUseableItem("sniper_extendedclip", function(source, item)
    local Player = MSCore.Functions.GetPlayer(source)
    TriggerClientEvent("weapons:client:EquipAttachment", source, item, "extendedclip")
end)

MSCore.Functions.CreateUseableItem("sniper_advscope", function(source, item)
    local Player = MSCore.Functions.GetPlayer(source)
    TriggerClientEvent("weapons:client:EquipAttachment", source, item, "advscope")
end)

MSCore.Functions.CreateUseableItem("sniper_scope", function(source, item)
    local Player = MSCore.Functions.GetPlayer(source)
    TriggerClientEvent("weapons:client:EquipAttachment", source, item, "scope")
end)

MSCore.Functions.CreateUseableItem("sniper_suppressor", function(source, item)
    local Player = MSCore.Functions.GetPlayer(source)
    TriggerClientEvent("weapons:client:EquipAttachment", source, item, "suppressor")
end)
...
MSCore.Functions.CreateUseableItem("camo", function(source, item)
    local Player = MSCore.Functions.GetPlayer(source)
        ...
        if Inventory[CurrentWeaponData.slot].info.attachments ~= nil and next(Inventory[CurrentWeaponData.slot].info.attachments) ~= nil then
            ...
            if not HasAttach then
                if not HasCamo then
                    ...
                    TriggerClientEvent("MSCore:Notify", src, "You already have a "..AttachmentData.label:lower().." on your weapon.", "error", 3500)
                end
            else
                TriggerClientEvent("MSCore:Notify", src, "You already have a "..AttachmentData.label:lower().." on your weapon.", "error", 3500)
            end
...
MSCore.Functions.CreateCallback('weapons:server:RemoveAttachment', function(source, cb, AttachmentData, ItemData)
    local src = source
    local Player = MSCore.Functions.GetPlayer(src)
    local Inventory = Player.PlayerData.items
    ...
    if Inventory[ItemData.slot] ~= nil then
        if Inventory[ItemData.slot].info.attachments ~= nil and next(Inventory[ItemData.slot].info.attachments) ~= nil then
            ...
            if HasAttach then
                ...
                TriggerClientEvent('inventory:client:ItemBox', src, MSCore.Shared.Items[AttachmentComponent.item], "add")
                TriggerClientEvent("MSCore:Notify", src, "You removed a "..AttachmentComponent.label.." from your weapon!", "error")
