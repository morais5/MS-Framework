...
AddEventHandler('MSCore:Client:OnPlayerLoaded', function()
    TriggerServerEvent("ms-clothes:loadPlayerSkin")
            ...
            for k, v in pairs(Config.Stores) do
                ...
                if dist < 30 then
                    if not creatingCharacter then
                        ...
                        if dist < 5 then
                            if Config.Stores[k].shopType == "clothing" then
                                DrawText3Ds(Config.Stores[k].x, Config.Stores[k].y, Config.Stores[k].z + 1.25, '~g~E~w~ - Buy clothes')
                            elseif Config.Stores[k].shopType == "barber" then
                                DrawText3Ds(Config.Stores[k].x, Config.Stores[k].y, Config.Stores[k].z + 1.25, '~g~E~w~ - Get a haircut')
                            end
                        ...
                        if dist < 2 then
                            if PlayerData.job.name == Config.ClothingRooms[k].requiredJob then
                                ...
                                if IsControlJustPressed(0, Keys["E"]) then
                                    ...
                                    gender = "male"
                                    if MSCore.Functions.GetPlayerData().charinfo.gender == 1 then gender = "female" end
                                    MSCore.Functions.TriggerCallback('ms-clothing:server:getOutfits', function(result)
                                        openMenu({
...
RegisterNetEvent('ms-clothing:client:openOutfitMenu')
AddEventHandler('ms-clothing:client:openOutfitMenu', function()
    MSCore.Functions.TriggerCallback('ms-clothing:server:getOutfits', function(result)
        openMenu({
...
RegisterNUICallback('selectOutfit', function(data)
    ...
    TriggerEvent('ms-clothing:client:loadOutfit', data)
end)
...
RegisterNetEvent('ms-clothing:client:openMenu')
AddEventHandler('ms-clothing:client:openMenu', function()
    customCamLocation = nil
    openMenu({
...
RegisterNUICallback('TrackerError', function()
    TriggerEvent('chatMessage', "SYSTEM", "error", "Its not possible to do this..")
end)
...
RegisterNUICallback('saveOutfit', function(data, cb)
    ...
    TriggerServerEvent('ms-clothes:saveOutfit', data.outfitName, model, skinData)
end)

RegisterNetEvent('ms-clothing:client:reloadOutfits')
AddEventHandler('ms-clothing:client:reloadOutfits', function(myOutfits)
    SendNUIMessage({
...
RegisterNUICallback('removeOutfit', function(data, cb)
    TriggerServerEvent('ms-clothing:server:removeOutfit', data.outfitName, data.outfitId)
...
function SaveSkin()
	...
	TriggerServerEvent("ms-clothing:saveSkin", model, clothing)
end

RegisterNetEvent('ms-clothes:client:CreateFirstCharacter')
AddEventHandler('ms-clothes:client:CreateFirstCharacter', function()
    MSCore.Functions.GetPlayerData(function(PlayerData)
        local skin = "mp_m_freemode_01"
        openMenu({
            {menu = "character", label = "Character", selected = true},
            {menu = "clothing", label = "Clothes", selected = false},
            {menu = "accessoires", label = "Accessories", selected = false}
        })
...
RegisterNetEvent("ms-clothes:loadSkin")
AddEventHandler("ms-clothes:loadSkin", function(new, model, data)
    ...
    Citizen.CreateThread(function()
        ...
        TriggerEvent('ms-clothing:client:loadPlayerClothing', data, GetPlayerPed(-1))
    end)
end)

RegisterNetEvent('ms-clothing:client:loadPlayerClothing')
AddEventHandler('ms-clothing:client:loadPlayerClothing', function(data, ped)
...
RegisterNetEvent('ms-clothing:client:loadOutfit')
AddEventHandler('ms-clothing:client:loadOutfit', function(oData)
    local ped = GetPlayerPed(-1)
...
RegisterNetEvent("ms-clothing:client:adjustfacewear")
AddEventHandler("ms-clothing:client:adjustfacewear",function(type)
