...
msClothing.ResetItemTexture = function(obj, category) {
    ...
    $.post('http://ms-clothing/updateSkin', JSON.stringify({
...
$(document).on('click', '.clothing-menu-option-item-right', function(e){
    ...
    if (canChange) {
        if (hasTracker && clothingCategory == "accessory") {
            $.post('http://ms-clothing/TrackerError');
            return
        } else {
            if (clothingCategory == "model") {
                $(inputElem).val(newValue);
                $.post('http://ms-clothing/setCurrentPed', JSON.stringify({ped: newValue}), function(model){
                    $("#current-model").html("<p>"+model+"</p>")
                });
                canChange = true;
                msClothing.ResetValues()
                ...
                if (buttonType == "item") {
                    msClothing.ResetItemTexture(this, clothingCategory);
                }
            } else {
                if (buttonType == "item") {
...
$(document).on('change', '.item-number', function(){
    ...
    if (hasTracker && clothingCategory == "accessory") {
        ...
        if (clothingCategory == "accessory" && inputVal == 13) {
            ...
            $.post('http://ms-clothing/updateSkinOnInput', JSON.stringify({
                clothingType: clothingCategory,
...
$(document).on('click', '.clothing-menu-header-camera-btn', function(e){
    ...
    if (selectedCam == null) {
        ...
        $.post('http://ms-clothing/setupCam', JSON.stringify({
            value: camValue
        }));
        selectedCam = this;
    } else {
...
$(document).on('keydown', function() {
    switch(event.keyCode) {
        case 68: // D
            $.post('http://ms-clothing/rotateRight');
            break;
        case 65: // A
            $.post('http://ms-clothing/rotateLeft');
            break;
        case 38: // UP
            ChangeUp();
            break;
...
msClothing.ToggleChange = function(bool) {
    canChange = bool;
}
...
$(document).ready(function(){
    window.addEventListener('message', function(event) {
        switch(event.data.action) {
            case "open":
                msClothing.Open(event.data);
                break;
            case "close":
                msClothing.Close();
                break;
            case "updateMax":
                msClothing.SetMaxValues(event.data.maxValues);
                break;
            case "reloadMyOutfits":
                msClothing.ReloadOutfits(event.data.outfits);
                break;
            case "toggleChange":
                msClothing.ToggleChange(event.data.allow);
                break;
            case "ResetValues":
                msClothing.ResetValues();
                break;
...
msClothing.ReloadOutfits = function(outfits) {
...
$(document).on('click', "#save-menu", function(e){
    e.preventDefault();
    msClothing.Close();
    $.post('http://ms-clothing/saveClothing');
});

$(document).on('click', "#cancel-menu", function(e){
    e.preventDefault();
    msClothing.Close();
    $.post('http://ms-clothing/resetOutfit');
});

msClothing.SetCurrentValues = function(clothingValues) {
...
msClothing.Open = function(data) {
    ...
    msClothing.SetMaxValues(data.maxValues);
    $(".clothing-menu-header").html("");
    msClothing.SetCurrentValues(data.currentClothing);
...
$(document).on('click', '.clothing-menu-outfit-option-button', function(e){
    ...
    $.post('http://ms-clothing/selectOutfit', JSON.stringify({
...
$(document).on('click', '.clothing-menu-myOutfit-option-button-remove', function(e){
    ...
    $.post('http://ms-clothing/removeOutfit', JSON.stringify({
        outfitData: outfitData.skin,
        outfitName: outfitData.outfitname,
        outfitId: outfitData.outfitId,
    }));
...
msClothing.Close = function() {
    $.post('http://ms-clothing/close');
...
msClothing.SetMaxValues = function(maxValues) {
...
msClothing.ResetValues = function() {
...
$(document).on('click', '#save-outfit-save', function(e){
    ...
    $.post('http://ms-clothing/saveOutfit', JSON.stringify({
...
$(document).on('click', '.change-camera-button', function(e){
    ...
    $.post('http://ms-clothing/rotateCam', JSON.stringify({
        type: rotationType
    }))
});
