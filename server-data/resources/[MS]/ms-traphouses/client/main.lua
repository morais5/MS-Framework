...
Citizen.CreateThread(function()
    ...
    if MSCore.Functions.GetPlayerData() ~= nil then
        ...
        MSCore.Functions.TriggerCallback('ms-traphouses:server:GetTraphousesData', function(trappies)
...
RegisterNetEvent('ms-traphouses:client:EnterTraphouse')
AddEventHandler('ms-traphouses:client:EnterTraphouse', function(code)
    if ClosestTraphouse ~= nil then
...
RegisterNUICallback('EnterPincode', function(d)
    ...
    if tonumber(d.pin) == data.pincode then
        ...
        MSCore.Functions.Notify('This code is not valide..', 'error')
    end
end)
                                    ...
                                    if not IsEntityDead(targetPed) then
                                        if CanRob then
                                            if InDistance then
                                                ...
                                                TriggerServerEvent('ms-traphouses:server:RobNpc', ClosestTraphouse)
                                                CanRob = false
                                            end
                                        end
                                    end
                ...
                if InteractDistance < 20 then
                    ...
                    if InteractDistance < 1 then
                        if not IsKeyHolder then
                            ...
                            if IsControlJustPressed(0, Keys["E"]) then
                                TriggerServerEvent('ms-traphouses:server:TakeoverHouse', CurrentTraphouse)
                            end
                            ...
                            if IsHouseOwner then
                                ...
                                if IsControlJustPressed(0, Keys["G"]) then
                                    MSCore.Functions.Notify('Pincode: '..data.pincode)
                                end
                            end
                            if IsControlJustPressed(0, Keys["H"]) then
                                local TraphouseInventory = {}
                            ...
                            if IsControlJustPressed(0, Keys["E"]) then
                                TriggerServerEvent("ms-traphouses:server:TakeMoney", CurrentTraphouse)
                            end
                        end
                    end
                end
...
function LeaveTraphouse(data)
    ...
    DoScreenFadeOut(250)
    Citizen.Wait(250)
    exports['ms-interior']:DespawnInterior(TraphouseObj, function()
...
RegisterNetEvent('ms-traphouses:client:TakeoverHouse')
AddEventHandler('ms-traphouses:client:TakeoverHouse', function(TraphouseId)
    local ped = GetPlayerPed(-1)

    MSCore.Functions.Progressbar("takeover_traphouse", "Overtaking traphouse", math.random(1000, 3000), false, true, {
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        disableCombat = true,
    }, {}, {}, {}, function() -- Done
        TriggerServerEvent('ms-traphouses:server:AddHouseKeyHolder', PlayerData.citizenid, TraphouseId, true)
    end, function()
        MSCore.Functions.Notify("Overtaking canceled..", "error")
    end)
end)
...
function AddKeyHolder(CitizenId, Traphouse)
    if #Config.TrapHouses[Traphouse].keyholders <= 6 then
        if not HasCitizenIdHasKey(CitizenId, Traphouse) then
            ...
            MSCore.Functions.Notify(CitizenId..' is added to the traphouse!')
        else
            MSCore.Functions.Notify(CitizenId..' This person does already have the keys!')
        end
    else
        MSCore.Functions.Notify('You can max give 6 other people acces to the trap house!')
    end
    IsKeyHolder = HasKey(CitizenId)
    IsHouseOwner = IsOwner(CitizenId)
end

RegisterNetEvent('ms-traphouses:client:SyncData')
AddEventHandler('ms-traphouses:client:SyncData', function(k, data)
