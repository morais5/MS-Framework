...
AddEventHandler('hospital:client:TakeBrancard', function()
    ...
    if Vehicle ~= 0 then
        ...
        if Obj ~= nil or Obj ~= 0 then
            ...
            MSCore.Functions.Notify("Something went wrong...", 'error')
        end
    else
        MSCore.Functions.Notify("You are not close to an ambulance..", 'error')
    end
end)
    ...
    if BrancardObject ~= nil then
        ...
        if Dist < 3.0 then
            if DoesEntityExist(BrancardObject) then
                ...
                TriggerServerEvent('ms-radialmenu:server:RemoveBrancard', PlayerPos, BrancardObject)
                IsAttached = false
                BrancardObject = nil
                IsLayingOnBed = false
            end
        else
            MSCore.Functions.Notify('You are too far!', 'error')
        end
    end
end)
...
RegisterNetEvent('ms-radialmenu:client:RemoveBrancardFromArea')
AddEventHandler('ms-radialmenu:client:RemoveBrancardFromArea', function(PlayerPos, BObject)
    local Ped = GetPlayerPed(-1)
...
function LayOnBrancard()
    ...
    if player == -1 then
        ...
        if distance < 2.0 then
            TriggerServerEvent('ms-radialmenu:Brancard:BusyCheck', GetPlayerServerId(player), "lay")
        else
            LoadAnim(inBedDicts)
            if Object ~= nil or Object ~= 0 then
                TaskPlayAnim(PlayerPedId(), inBedDicts, inBedAnims, 8.0, 8.0, -1, 69, 1, false, false, false)
...
RegisterNetEvent('ms-radialmenu:Brancard:client:BusyCheck')
AddEventHandler('ms-radialmenu:Brancard:client:BusyCheck', function(OtherId, type)
    local ped = GetPlayerPed(-1)
    if type == "lay" then
        ...
        if IsEntityPlayingAnim(ped, "anim@gangops@morgue@table@", "ko_front", 3) then
            TriggerServerEvent('ms-radialmenu:server:BusyResult', true, OtherId, type)
        else
            TriggerServerEvent('ms-radialmenu:server:BusyResult', false, OtherId, type)
        end
    else
...
RegisterNetEvent('ms-radialmenu:client:Result')
AddEventHandler('ms-radialmenu:client:Result', function(IsBusy, type)
    local inBedDicts = "anim@gangops@morgue@table@"
    local inBedAnims = "ko_front"
    local PlayerPed = PlayerPedId()
    local PlayerPos = GetEntityCoords(PlayerPed)
    ...
    if type == "lay" then
        if not IsBusy then
            ...
            MSCore.Functions.Notify("This stretcher is already in use!", "error")
            IsLayingOnBed = false
        end
    else
        if not IsBusy then
...
function AttachToBrancard()
    ...
    if BrancardObject ~= nil then
        if ClosestPlayer == -1 then
            ...
            if distance < 2.0 then
                TriggerServerEvent('ms-radialmenu:Brancard:BusyCheck', GetPlayerServerId(ClosestPlayer), "attach")
            else
                NetworkRequestControlOfEntity(BrancardObject)
                LoadAnim("anim@heists@box_carry@")
                TaskPlayAnim(PlayerPed, 'anim@heists@box_carry@', 'idle', 8.0, 8.0, -1, 50, 0, false, false, false)
