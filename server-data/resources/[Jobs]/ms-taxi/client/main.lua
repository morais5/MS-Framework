...
RegisterNetEvent('ms-taxi:client:DoTaxiNpc')
AddEventHandler('ms-taxi:client:DoTaxiNpc', function()
    if whitelistedVehicle() then
        --if NpcData.CountDown == 180 then
            if not NpcData.Active then
                ...
                MSCore.Functions.Notify('The NPC is indicated on your navigation!', 'success')
                        ...
                        if dist < 20 then
                            ...
                            if dist < 5 then
                                ...
                                if IsControlJustPressed(0, Keys["E"]) then
                                    ...
                                    MSCore.Functions.Notify('Bring the NPC to the specified location.')
                                    if NpcData.NpcBlip ~= nil then
                                        RemoveBlip(NpcData.NpcBlip)
                                    end
                                    GetDeliveryLocation()
                ...
                MSCore.Functions.Notify('You are already doing an NPC mission..')
            end
    else
        MSCore.Functions.Notify('You are not in a Taxi :(')
    end
end)
            ...
            if dist < 20 then
                ...
                if dist < 5 then
                    ...
                    if IsControlJustPressed(0, Keys["E"]) then
                        ...
                        TriggerServerEvent('ms-taxi:server:NpcPay', meterData.currentFare)
                        MSCore.Functions.Notify('Person in top condition', 'success')
                        if NpcData.DeliveryBlip ~= nil then
                            RemoveBlip(NpcData.DeliveryBlip)
                        end
...
RegisterNetEvent('ms-taxi:client:toggleMeter')
AddEventHandler('ms-taxi:client:toggleMeter', function()
    local ped = GetPlayerPed(-1)
    ...
    if IsPedInAnyVehicle(ped, false) then
        if whitelistedVehicle() then
            ...
            MSCore.Functions.Notify('This vehicle has no Taxi Meter..', 'error')
        end
    else
...
RegisterNetEvent('ms-taxi:client:enableMeter')
AddEventHandler('ms-taxi:client:enableMeter', function()
    local ped = GetPlayerPed(-1)
    ...
    if meterIsOpen then
        ...
        MSCore.Functions.Notify('The Taxi Meter is not active..', 'error')
    end
end)
...
RegisterNetEvent('ms-taxi:client:toggleMuis')
AddEventHandler('ms-taxi:client:toggleMuis', function()
    Citizen.Wait(400)
    if meterIsOpen then
        ...
        MSCore.Functions.Notify('No Taxi Meter in sight..', 'error')
    end
end)
...
function TakeVehicle(k)
    ...
    MSCore.Functions.SpawnVehicle(Config.AllowedVehicles[k].model, function(veh)
