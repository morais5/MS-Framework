...
AddEventHandler('QBCore:Client:OnPlayerLoaded', function()
    ...
    isLoggedIn = true
    QBCore.Functions.TriggerCallback('qb-food:server:GetAttachedVehicle', function(plates)
        for k, v in pairs(plates) do
            Config.Plates[k].AttachedVehicle = v.AttachedVehicle
        end
    end)

    QBCore.Functions.TriggerCallback('qb-food:server:GetDrivingDistances', function(retval)
                                    ...
                                    if IsPedInAnyVehicle(GetPlayerPed(-1)) then
                                        if not IsThisModelABicycle(GetEntityModel(veh)) then
                                            ...
                                            if IsControlJustPressed(0, Config.Keys["E"]) then
                                                ...
                                                Wait(500)
                                                DoScreenFadeIn(250)
                                                TriggerServerEvent('qb-food:server:SetAttachedVehicle', veh, k)
                                            end
                                        else
            ...
            if v.name == PartData.item then
                if v.amount >= PartData.costs then
                    QBCore.Functions.Progressbar("repair_part", Config.ValuesLabels[part].." repairing", math.random(5000, 10000), false, true, {
                        ...
                        TriggerEvent('qb-food:client:RepaireeePart', part)
                        TriggerServerEvent('qb-inventory:server:SaveStashItems', "foodstash", StashItems)
...
RegisterNetEvent('qb-food:client:RepaireeePart')
AddEventHandler('qb-food:client:RepaireeePart', function(part)
...
function UnattachVehicle()
    ...
    TriggerServerEvent('qb-food:server:SetAttachedVehicle', false, ClosestPlate)
end

RegisterNetEvent('qb-food:client:SetAttachedVehicle')
AddEventHandler('qb-food:client:SetAttachedVehicle', function(veh, key)
    if veh ~= false then
            ...
            if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, vehpos.x, vehpos.y, vehpos.z, true) < 5.0 then
                if not IsThisModelABicycle(GetEntityModel(veh)) then
                    ...
                    if VehicleStatus[plate] ~= nil then 
                        ...
                        QBCore.Functions.Notify("Ga status known ..", "error")
                    end
                else
...
function CanReapair()
    ...
    for k, v in pairs(Config.Businesses) do
        retval = exports['qb-businesses']:IsEmployee(v)
    end
    return retval
end
