...
AddEventHandler('MSCore:Client:OnPlayerLoaded', function()
    MSCore.Functions.GetPlayerData(function(PlayerData)
        ...
        if PlayerData.job.onduty then
            if PlayerData.job.name == "food" then
                TriggerServerEvent("MSCore:ToggleDuty")
            end
        end
    end)
    isLoggedIn = true
    MSCore.Functions.TriggerCallback('ms-food:server:GetAttachedVehicle', function(plates)
        for k, v in pairs(plates) do
            Config.Plates[k].AttachedVehicle = v.AttachedVehicle
        end
    end)

    MSCore.Functions.TriggerCallback('ms-food:server:GetDrivingDistances', function(retval)
...
Citizen.CreateThread(function()
    ...
    AddTextComponentSubstringPlayerName("Shop Food")
                    ...
                    if StashDistance < 20 then
                        ...
                        if StashDistance < 1 then
                            ...
                            if IsControlJustReleased(0, Keys["E"]) then
                                TriggerEvent("inventory:client:SetCurrentStash", "foodstash")
                                TriggerServerEvent("inventory:server:OpenInventory", "stash", "foodstash", {
                                    maxweight = 4000000,
                                    slots = 500,
                                })
                            end
                                    ...
                                    if IsPedInAnyVehicle(GetPlayerPed(-1)) then
                                        if not IsThisModelABicycle(GetEntityModel(veh)) then
                                            ...
                                            if IsControlJustPressed(0, Config.Keys["E"]) then
                                                ...
                                                Wait(500)
                                                DoScreenFadeIn(250)
                                                TriggerServerEvent('ms-food:server:SetAttachedVehicle', veh, k)
                                            end
                                        else
                                            MSCore.Functions.Notify("You cannot put bicycles on the plate!", "error")
                                        end
                                    end
                                end
                            end
        ...
        for k, v in pairs(StashItems) do
            if v.name == PartData.item then
                if v.amount >= PartData.costs then
                    MSCore.Functions.Progressbar("repair_part", Config.ValuesLabels[part].." repairing", math.random(5000, 10000), false, true, {
                        ...
                        TriggerEvent('ms-food:client:RepaireeePart', part)
                        TriggerServerEvent('ms-inventory:server:SaveStashItems', "foodstash", StashItems)
                        SetTimeout(250, function()
                            PartsMenu()
                        end)
                    end, function()
                        MSCore.Functions.Notify("Repair canceled..", "error")
                    end)
                    break
                else
                    MSCore.Functions.Notify('There are not enough materials in the safe ..', 'error')
                end
                break
            else
                MSCore.Functions.Notify('There are not enough materials in the safe ..', 'error')
...
RegisterNetEvent('ms-food:client:RepaireeePart')
AddEventHandler('ms-food:client:RepaireeePart', function(part)
    ...
    MSCore.Functions.Notify("The "..Config.ValuesLabels[part].." is repaired!")
end)
...
function UnattachVehicle()
    ...
    TriggerServerEvent('ms-food:server:SetAttachedVehicle', false, ClosestPlate)
end

RegisterNetEvent('ms-food:client:SetAttachedVehicle')
AddEventHandler('ms-food:client:SetAttachedVehicle', function(veh, key)
    if veh ~= false then
            ...
            if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, vehpos.x, vehpos.y, vehpos.z, true) < 5.0 then
                if not IsThisModelABicycle(GetEntityModel(veh)) then
                    ...
                    if VehicleStatus[plate] ~= nil then 
                        ...
                        MSCore.Functions.Notify("Ga status known ..", "error")
                    end
                else
                    MSCore.Functions.Notify("Not a valid vehicle ..", "error")
                end
            else
                MSCore.Functions.Notify("You are not close enough to the vehicle..", "error")
            end
        else
            MSCore.Functions.Notify("You must be in the vehicle first.", "error")
        end
    else
        MSCore.Functions.Notify("You must be outside the vehicle..", "error")
    end
end)
...
AddEventHandler('vehiclemod:client:fixEverything', function()
    if (IsPedInAnyVehicle(GetPlayerPed(-1), false)) then
        ...
        if not IsThisModelABicycle(GetEntityModel(veh)) and GetPedInVehicleSeat(veh, -1) == GetPlayerPed(-1) then
            ...
            MSCore.Functions.Notify("You are not a driver or on a bicycle ..", "error")
        end
    else
        MSCore.Functions.Notify("You're not in a vehicle ..", "error")
    end
end)
                ...
                if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, vehpos.x, vehpos.y, vehpos.z, true) < 5.0 then
                    if not IsThisModelABicycle(GetEntityModel(veh)) then
                        ...
                        if VehicleStatus[plate] ~= nil and VehicleStatus[plate][part] ~= nil then
                            ...
                            MSCore.Functions.Progressbar("repair_advanced", "Repair Vehicle..", lockpickTime, false, true, {
                                disableMovement = true,
                                disableCarMovement = true,
                                disableMouse = false,
                                disableCombat = true,
                            ...
                            MSCore.Functions.Notify("Not a valid part ..", "error")
                        end
                    else
                        MSCore.Functions.Notify("Not a valid vehicle ..", "error")
                    end
...
function ScrapAnim(time)
    ...
    TaskPlayAnim(GetPlayerPed(-1), "mp_car_bomb", "car_bomb_food" ,3.0, 3.0, -1, 16, 0, false, false, false)
    openingDoor = true
    Citizen.CreateThread(function()
        while openingDoor do
            TaskPlayAnim(PlayerPedId(), "mp_car_bomb", "car_bomb_food", 3.0, 3.0, -1, 16, 0, 0, 0, 0)
            Citizen.Wait(2000)
            time = time - 2
            if time <= 0 then
                openingDoor = false
                StopAnimTask(GetPlayerPed(-1), "mp_car_bomb", "car_bomb_food", 1.0)
            end
        end
    end)
end
...
function CanReapair()
    ...
    for k, v in pairs(Config.Businesses) do
        retval = exports['ms-businesses']:IsEmployee(v)
    end
    return retval
end
