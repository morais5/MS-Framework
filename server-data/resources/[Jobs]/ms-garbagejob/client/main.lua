        ...
        while Earnings > 0 do
            ...
            if distance < 20 then
                ...
                if distance < 1.5 then
                    DrawText3D(coords.x, coords.y, coords.z, "~g~E~w~ - Receive payment")
                    if IsControlJustPressed(0, Keys["E"]) then
                        TriggerServerEvent('ms-garbagejob:server:PayShit', Earnings, location)
                        Earnings = 0
                    end
                elseif distance < 5 then
                    DrawText3D(coords.x, coords.y, coords.z, "Receive payment")
                end
            end
        ...
        if isLoggedIn then
            if PlayerJob.name == "garbage" then
                if distance < 10.0 then
                    ...
                    if distance < 1.5 then
                        if InVehicle then
                            DrawText3D(Config.Locations["vehicle"].coords.x, Config.Locations["vehicle"].coords.y, Config.Locations["vehicle"].coords.z, "~g~E~w~ -  Deliver vehicle")
                            if IsControlJustReleased(0, Keys["E"]) then
                                MSCore.Functions.TriggerCallback('ms-garbagejob:server:CheckBail', function(DidBail)
                                    if DidBail then
                                        BringBackCar()
                                        MSCore.Functions.Notify("You have received your $1000!")
                                    else
                                        MSCore.Functions.Notify("You have not paid any deposit on this vehicle..")
                                    end
                                end)
                            end
                        else
                            DrawText3D(Config.Locations["vehicle"].coords.x, Config.Locations["vehicle"].coords.y, Config.Locations["vehicle"].coords.z, "~g~E~w~ - Require vehicle")
                            if IsControlJustReleased(0, Keys["E"]) then
                                MSCore.Functions.TriggerCallback('ms-garbagejob:server:HasMoney', function(HasMoney)
                                    if HasMoney then
                                        local coords = Config.Locations["vehicle"].coords
                                        MSCore.Functions.SpawnVehicle("trash2", function(veh)
                                            GarbageVehicle = veh
                                            ...
                                            IsWorking = true
                                            SetGarbageRoute()
                                            MSCore.Functions.Notify("You paid $1000 as a deposit for the vehicle!")
                                            MSCore.Functions.Notify("You started your route, a location was set on your GPS!")
                                        end, coords, true)
                                    else
                                        MSCore.Functions.Notify("You don't have enough money for the deposit ... The deposit costs "..Config.BailPrice)
                                    end
                                end)
                            end
                        end
                        ...
                        if DeliveryBlip ~= nil then
                            ...
                            if Distance < 20 or hasZak then
                                ...
                                if not hasZak then
                                    if CanTakeBag then
                                        if Distance < 1.5 then
                                            DrawText3D2(DeliveryData.coords, "~g~E~w~ - Pick up the trash")
                                        ...
                                        elseif Distance < 10 then
                                            DrawText3D2(DeliveryData.coords, "Approach to pick up the trash.")
                                        end
                                    end
                                else
                                    if DoesEntityExist(GarbageVehicle) then
                                        if Distance < 10 then
                                            DrawText3D2(DeliveryData.coords, "Put the trash in the garbage truck...")
                                        end
                                        ...
                                        if TruckDist < 2 then
                                            DrawText3D(Coords.x, Coords.y, Coords.z, "~g~E~w~ - Throwing the trash bag")
                                            if IsControlJustPressed(0, 51) then
                                                hasZak = false
                                                ...
                                                if (AmountOfBags - 1) == 0 then
                                                    ...
                                                    if (GarbageLocation + 1) <= AmountOfLocations then
                                                        ...
                                                        SetGarbageRoute()
                                                        MSCore.Functions.Notify("The container has been cleaned, move on to the next location!")
                                                    else
                                                        -- Hier ben je klaar met werken.
                                                        MSCore.Functions.Notify("You finished the shift, go back to central.")
                                                        IsWorking = false
                                                        RemoveBlip(DeliveryBlip)
                                                        SetRouteBack()
                                                    end
                                                    ...
                                                    if AmountOfBags > 1 then
                                                        MSCore.Functions.Notify("There are still"..AmountOfBags.." bags left!")
                                                    else
                                                        MSCore.Functions.Notify("Still missing "..AmountOfBags.." bag!")
                                                    end
                                                    hasZak = false
                                                end
                                                DeliverAnim()
                                        ...
                                        elseif TruckDist < 10 then
                                            DrawText3D(Coords.x, Coords.y, Coords.z, "Come here..")
                                        end
                                    else
