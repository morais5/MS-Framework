        ...
        if IsControlJustPressed(0, Config.OpenPhone) then
            if not PhoneData.isOpen then
                ...
                if not IsHandcuffed then
                    ...
                    QBCore.Functions.Notify("Não podes fazer isso neste momento", "error")
                end
            end
        end
        Citizen.Wait(3)
...
function OpenPhone()
    QBCore.Functions.TriggerCallback('qb-phone_new:server:HasPhone', function(HasPhone)
        if HasPhone then
            ...
            QBCore.Functions.Notify("Não tens um telemóvel", "error")
        end
    end)
end
...
RegisterNUICallback('SetAlertWaypoint', function(data)
    ...
    QBCore.Functions.Notify('Localização marcada no GPS para: '..data.alert.title)
...
RegisterNUICallback('RaceDistanceCheck', function(data, cb)
    QBCore.Functions.TriggerCallback('qb-lapraces:server:GetRacingData', function(RaceData)
        ...
        if dist <= 115.0 then
            ...
            QBCore.Functions.Notify('Estás muito longe da corrida. A localização foi marcada no teu GPS.', 'error', 5000)
...
RegisterNUICallback('SetHouseLocation', function(data, cb)
    ...
    QBCore.Functions.Notify("O teu GPS foi marcado para " .. data.HouseData.HouseData.adress .. "!", "success")
end)
...
RegisterNUICallback('GetCurrentMecanicos', function(data, cb)
    QBCore.Functions.TriggerCallback('qb-phone_new:server:GetCurrentMecanicos', function(mecanicos)
        cb(mecanicos)
    end)
end)
